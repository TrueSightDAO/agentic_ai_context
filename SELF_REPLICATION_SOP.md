...
| **Fork** | A completely new ecosystem: new Sophia + new Edgar + new DB + new ledger + blank context | New DAO, new rules (e.g. Liz wants to run a coffee co-op with different tokenomics) |
...
| **New Edgar** | Fresh `sentiment_importer` (Rails API) instance |
| **New PostgreSQL** | Empty database with schema migrations run |
| **New ledger** | Blank Google Sheet (or alternative) for the new DAO's transactions |
...
2. **Provision infrastructure** — EC2 for Sophia + Edgar, RDS for PostgreSQL (or separate DB server)
3. **Clone both repos** — `truesight_autopilot` + `sentiment_importer`
4. **Set up Edgar** — `rails db:create db:migrate`, configure API keys
...
8. **Start both services** — Sophia + Edgar
9. **Governor provisions credentials** — visits new vault UI
10. **Verify** — `/vault/status` shows both Sophia and Edgar as healthy
...
- [ ] PostgreSQL has schema migrated
...
- Forking Edgar requires PostgreSQL and Redis — the target server needs those installed or accessible.
