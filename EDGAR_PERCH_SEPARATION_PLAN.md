...
Let's Encrypt cert obtained via certbot on seni_ror (the Rails box). Auto-renewal
configured. Cert stored at:
- `/etc/letsencrypt/live/perch.truesight.me/fullchain.pem`
- `/etc/letsencrypt/live/perch.truesight.me/privkey.pem`

**Status:** ✅ Done (2026-06-13). Expires 2026-09-11.

### 1.3 Add nginx server block on seni_ror

Created `/etc/nginx/sites-available/perch.conf` — static root at `/var/www/perch`,
serving the landing page. Enabled and reloaded. Config:
...
**Status:** ✅ Done. `https://perch.truesight.me` loads with valid SSL.
...
**Status:** ✅ PRs open on `truesight_me_beta`:
- PR #214: `perch.html` + updated `edgar.html`
- PR #215: Navigation updated on all 8 pages
...
8. ⏳ All existing automated consumers continue to work without changes
...
- ✅ SSL cert (Let's Encrypt, auto-renewal)
- ✅ Nginx config (static landing page)
- ✅ `https://perch.truesight.me` live
- ✅ PRs open for website changes (#214, #215)

Next:
- ⏳ Deploy Edgar's protocol landing page (1.4)
- ⏳ Wire Rails domain-aware routing (1.5)
- ⏳ Merge website PRs and sync to prod
- ⏳ Phase 2: Stripe webhook split
