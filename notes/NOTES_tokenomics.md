...
- **GTIN vs QR (product identity):** the retail **GTIN identifies the product TYPE, not the unit/vintage** — **all chocolate bars share one GTIN; all ceremonial cacao share one GTIN**. The **serialized QR (column A)** is the per-unit identity and the only thing that resolves **farm + vintage** / provenance. A vintage-independent "generic" SKU reuses the existing shared GTIN (never mint one per vintage); vintage product pages are sub-views of one GTIN. Full note: **`AGROVERSE_QR_CODE_BATCH_GENERATION.md` §0**.
