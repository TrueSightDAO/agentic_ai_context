...
3. **Git archive (Beer Hall — default for routine publishes):** After the sheet row exists, write a **Markdown + YAML frontmatter** entry into [**TrueSightDAO/ecosystem_change_logs**](https://github.com/TrueSightDAO/ecosystem_change_logs) so the same digest is **versioned for review** and can later be **surfaced on truesight.me**. **`cd`** to your **`ecosystem_change_logs`** clone, with Message 1 and Message 2 bodies in temp files (same text as WhatsApp), then run **`python3 scripts/archive_beer_hall_changelog.py --slug <short-hint> --tldr-file … --message2-file …`** plus the same **`--links`**, **`--pr-commit-links`**, **`--openclaw-message-id`**, and **`--notes`** you used for the sheet row. Then **`git` commit** and **merge** (PR or direct to `main`, per your governance). Format rationale and layout: see **`ecosystem_change_logs/README.md`**. *Skip only when the operator explicitly waives git archive (e.g. emergency) or the clone is unavailable — then record that in sheet **`notes`**.*

4. **LLM advisory corpus (pair with Beer Hall review — not a substitute for steps 1–3):** Whenever an agent **prepares or reviews** Beer Hall / Founder Haus digest evidence (including **`generate_beer_hall_preview.py`** in **§ Preview digest**), also run **`python3 scripts/generate_advisory_snapshot.py`** from **`market_research/`** so **`agentic_ai_context/ADVISORY_SNAPSHOT.md`** and **`ecosystem_change_logs/advisory/snapshots/<UTC-date>.md`** stay current for **LLM advisors** and Cursor sessions. Use the same **`--since-days`** window as the preview when practical; add **`--with-sheet-sales`** when **`market_research/google_credentials.json`** is present so **Monthly Statistics** + **QR Code Sales** roll into the snapshot (see **`tokenomics/SCHEMA.md`**). Git **commit/push** of those outputs follows your normal advisory workflow—this step does **not** replace the WhatsApp send, sheet row, or git archive above.
...
2. **Same session — LLM advisory snapshot:** Run **`python3 scripts/generate_advisory_snapshot.py`** from **`market_research/`** (match **`--since-days`** to the preview when practical). Add **`--with-sheet-sales`** when **`google_credentials.json`** exists so rollup + **QR Code Sales** evidence is inlined for advisors (**`tokenomics/SCHEMA.md`**). Writes **`ADVISORY_SNAPSHOT.md`** and **`../ecosystem_change_logs/advisory/snapshots/<UTC-date>.md`**. See **§ Closed loop** step **4** and the script’s module docstring.
3. The Markdown includes **placeholders** for **Message 1** (TLDR only) and **Message 2** (*Shipped* + links), then **raw evidence**: per-repo **`git log`** on each local **`TrueSightDAO`** clone in the script’s table, optional **`gh pr list --state merged`** excerpts, the **Telegram Chat Logs** helper output (same workbook as **§ Gathering Telegram Chat Logs**), and the **DApp Remarks** helper output (Hit List workbook; **§ Gathering DApp Remarks**).
4. An assistant or the operator **edits** the two messages into **WhatsApp-safe** text (**[§ WhatsApp-safe formatting](#whatsapp-safe-formatting)**; **[two-message rule](#beer-hall--two-whatsapp-messages-mandatory)**). The file may also contain a **suggested draft** for a specific session (still subject to human edit before any send).
5. **Only after approval:** send with **`openclaw message send`** (Beer Hall: **two** sequential messages) and append **`append_openclaw_beer_hall_log.py`** per **§ Closed loop**. A **preview run alone** does **not** replace steps 6–7 in the numbered list above.
