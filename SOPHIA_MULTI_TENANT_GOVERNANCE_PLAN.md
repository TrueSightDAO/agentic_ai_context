...
| **B — Phase 3: Vault store** | ✅ #165 | ✅ Merged | ☐ | P3.1–P3.7 |
| **B2 — Vault web page + tools** | ✅ #166 | ✅ Merged | ☐ | P3.3–P3.6 |
| **B3 — Safe deploy orchestration** | ✅ #167 | ✅ Merged | ☐ | — |
| **B4 — Heartbeat hooks** | ✅ #171 | ✅ Merged | ☐ | — |
| **D — Phase 1: Identity binding** | ✅ #172 | ✅ Merged | ☐ | P1.1–P1.5 |
| **C — Phase 0.2–0.4: Tool enforcement** | ☐ | ☐ | ☐ | P0.2–P0.4 |
| **E — Phase 2: Engagement modes** | ☐ | ☐ | ☐ | P2.1–P2.4 |
| **F — Phase 4: Multi-org replication** | ☐ | ☐ | ☐ | P4.1–P4.3 |

> **RESUME HERE (2026-06-11):** Phase 3 complete (store, web page, tools, safe deploy,
> heartbeat hooks). Phase 1 complete (identity binding). Next: remaining phases —
> C (Phase 0.2–0.4 tool enforcement), E (Phase 2 engagement modes), F (Phase 4
> multi-org replication). Open PRs via git worktree; **do not self-merge** (human
...
### Implementation (PRs #167, #171)

1. `app/deploy_watcher.py` — shared state file reader + `can_deploy()` logic
2. Wire heartbeat writes into: `email_poller`, `aws_monitor`, `_pending_janitor_loop`,
   `_context_sync_loop`, `_branch_janitor_loop`, `_self_heal_loop`
...
5. Tests: can_deploy blocks when track is active; proceeds when idle; proceeds when
