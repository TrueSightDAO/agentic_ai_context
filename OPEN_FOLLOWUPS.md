...
matching `Currencies`-tab row — which is exactly how **AUM valuation** (`tdg_wix_dashboard.js`
converts every AGL balance to USD via `Currencies!B`) and **first-seen** resolution look up
prices. Confirmed while refuting Decision 0.13: the snapshot's uppercase
`CACAO TREE TO BE PLANTED` was mistaken for a *live per-ledger literal* when the raw sheets
actually hold mixed case on every ledger.

**To do.** Either (a) drop the `.upper()` and snapshot the currency verbatim (then fix any
downstream code that relied on the uppercasing), or (b) keep the uppercasing but also emit the
raw value (e.g. `currency_raw`) so consumers can key correctly. Decide the option with the
governor; a case-insensitive lookup on the consumer side is the minimum safe fix. Add a
regression test that a mixed-case literal survives the snapshot round-trip.
