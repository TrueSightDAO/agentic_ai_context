...
### SunMint satellite cache: CDSE registration + secrets + collection verification
**Filed 2026-08-30. Owner: Gary (registration) → Sophia (wiring).** Scaffold is LIVE on the sunmint repo: `scripts/cache_satellite_scenes.py` + `.github/workflows/cache-satellite-scenes.yml` (daily 06:30 UTC after the tree-index rebuild). The script reads `trees/index.geojson`, buckets trees into ~1 km grid cells, queries CDSE STAC (`https://catalogue.dataspace.copernicus.eu/stac/search` — verified live; token endpoint `identity.dataspace.copernicus.eu` verified reachable) for recent low-cloud Sentinel-2 scenes, downloads small previews into `satellite/<lat>_<lng>/<scene-date>.jpg` + `satellite/manifest.json`. It degrades gracefully (exits 0 without creds — workflow never fails). **To enable:** (1) Gary registers at https://dataspace.copernicus.eu → My Account → create an OAuth2 client (confidential) → client_id + client_secret; (2) add them as sunmint repo secrets `CDSE_CLIENT_ID` / `CDSE_CLIENT_SECRET`; (3) verify the Sentinel-2 collection id on the new catalogue (the `SENTINEL-2` id 400s — collections are per-account; script tries `sentinel-2-l2a`, `sentinel-2`, etc.); (4) once scenes cache, add the date-slider history view to the truesight_me_beta impact map (#319) layered from `satellite/manifest.json`. Verified endpoints as of 2026-08-30: `catalogue.dataspace.copernicus.eu` (new STAC, `SENTINEL-2` collection not yet exposed publicly), legacy `stac.dataspace.copernicus.eu` deprecated 2025-11-17.
