...
### `define_currency.html` catalogs: automate the existing-but-never-run ledger snapshot + add the missing currency-fields cache — retire the live GAS round-trips
**Filed 2026-09-13. Owner: unclaimed. Governor: Gary (thread 27015).**

**Context.** `define_currency.html` loads three catalogs on every page open. Two round-trip to the DAO Forms GAS web app (`AKfycbztpV3TUIRn…`): the **ledger dropdown** (`?ledgers=true`) and the **Farm/State/Country seed lists** (`?currency_fields=true`). Measured from the autopilot box (3 runs each, 2026-09-13): `skus.json` raw-CDN **57–100 ms** (JSON cache, `cache-control: max-age=300`, ETag) vs GAS `?ledgers=true` **1.9–2.6 s** and `?currency_fields=true` **1.5–1.8 s** — i.e. ~4.5–5 s of "Loading catalogs…" per visit. The GAS handler (`tokenomics/google_app_scripts/1QtK-InsH…/web_app.js`) does a live `SpreadsheetApp.openById()` + `getRange().getValues()` read per call, so each pays container cold-start plus sheet I/O; it is **not** a client-side caching problem.

**Root cause is NOT a missing cache — corrected 2026-09-13.** A ledger cache **already exists**: `treasury-cache/managed-ledgers/_index.json` (+ per-ledger `<ID>.json`), built by `tokenomics/python_scripts/tdg_asset_management/snapshot_managed_ledgers.py`, which reads the **same** "Shipment Ledger Listing" sheet the GAS reads. But it is an **unautomated manual laptop script**: `OUTPUT_DIR=~/Applications/treasury-cache/managed-ledgers`, creds from `~/Applications/sentiment_importer/config/cypher_defense_gdrive_key.json`; **no** GHA/Apps-Script cron invokes it (tokenomics workflows are only daily-buyback, edgar-billing, hourly-tdg-usdc, 2 QR webhooks). Last generated **2026-06-02** (~3 months stale) and holds **13 ledgers vs 19 live** (it also skips `COMPLETED`/`SUSPENDED`). So the ledger feed went dark because a hand-run script **stopped being run** — not because no cache was ever designed. Contrast: the *rest* of `treasury-cache` **is** automated — `dao_offchain_treasury.json` / `dao_members.json` ship via a GAS publisher with a **30-min safety-net cron** + event triggers. The managed-ledgers files are the only piece still living on a laptop.

The **Farm/State/Country** seeds are the one genuinely **missing** artifact: no JSON cache holds them anywhere. `agroverse-inventory/currencies.json` exists (146 rows, daily cron `publish-agroverse-currencies.yml`, fresh) but carries only currency **names** (Currencies col A), not the G/H/I farm/state/country lists.

**Why it matters.** Both catalogs are *better* snapshot candidates than SKUs: ledgers change rarely, and the farm/state/country lists only change when a new value is first defined (low-frequency, append-mostly). And the ledger half is close to free — the snapshot script and the `_index.json` shape already exist; they just live on one laptop instead of a schedule. Leaving it manual means it silently goes stale the next time nobody runs it (which is exactly what happened between June and September).

**Proposed work.** Two parts, reusing what already exists rather than inventing:

1. **Ledgers — automate the existing script, don't rewrite it.** Give `snapshot_managed_ledgers.py` the **same GAS-publisher-plus-cron treatment the rest of `treasury-cache` already uses** (a time-triggered publisher for cheapness, or a daily GHA job mirroring `publish-agroverse-inventory-snapshot.yml` — either works; the GHA route needs the creds moved out of `~/Applications/...` into repo secrets). **Reuse the existing `managed-ledgers/_index.json` shape** (`schema_version`, `generated_at`, `source`, `ledgers[]` with `ledger_id`/`ledger_url`/`summary`) so any existing consumer keeps working — do **not** mint a new schema. Resolve the 13-vs-19 gap (decide whether `SUSPENDED` ledgers belong in the dropdown) in the same change. Then **repoint `loadLedgers()`** at the raw-CDN `_index.json`, keeping the GAS path as a graceful fallback.

2. **Farm/State/Country — add the missing catalog.** Emit **`currency-fields.json`** (distinct non-empty Currencies cols G/H/I) from the same snapshot job, and repoint `loadCurrencyFieldOptions()` at it, again with the GAS fallback (same degrade-to-empty shape as the SKU loader).

Expected once both land: all three catalogs ≈60 ms → init ~4.7 s → **~0.2 s**.
...
**Evidence.** `define_currency.html` L228 (`SKUS_JSON_URL`), L283 (`loadSkus`), L379 (`?ledgers=true`), L415 (`?currency_fields=true`) — line nos. pre-dapp_beta#94; `tokenomics/google_app_scripts/1QtK-InsHH6SBtxoxc33-y4vQvuNkbhlkUi_9S1X-AaEgIlSlygM1iZtP/web_app.js` (live-sheet read); **existing ledger cache** `treasury-cache/managed-ledgers/_index.json` (`generated_at 2026-06-02T20:30:03Z`, 13 ledgers) + producer `tokenomics/python_scripts/tdg_asset_management/snapshot_managed_ledgers.py` (`OUTPUT_DIR=~/Applications/treasury-cache/managed-ledgers`; `SKIP_STATUSES={'COMPLETED','SUSPENDED'}`; no workflow invokes it) vs GAS `?ledgers=true` returning **19**; `treasury-cache/README.md` (other caches auto-published via GAS publisher + 30-min cron); `agroverse-inventory` tree (has `currencies.json` = names only, `skus.json`; no `ledgers.json`/`currency-fields.json`); timing runs 2026-09-13; thread 27015.
