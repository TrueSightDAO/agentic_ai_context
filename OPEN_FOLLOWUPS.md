...
**Cleanup options (pick one):** (a) add a prune step to rebuild-plots-index.yml / cache-satellite-scenes.yml (delete plot_*/by-plot files not in the fresh feature set before push) — recommended, makes the workflow self-healing; (b) Contents-API DELETE of stale files; (c) accept as inert residue (manifest is authoritative; nothing references stale dirs).
