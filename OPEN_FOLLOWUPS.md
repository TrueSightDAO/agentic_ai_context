...
### truesight_autopilot_transcript: usage.jsonl / meta.json / messages.jsonl planned-but-not-implemented
**Filed 2026-08-31. Owner: unclaimed.** The transcript repo's own `AGENTS.md`/`ROADMAP.md` define
`sessions/<date>/<sid>/meta.json`, `messages.jsonl`, `usage.jsonl`, and `usage/<date>/_daily_summary.json`
as canonical per-call token/cost records — but **none are written yet** (verified 2026-08-31: session
`0ea2405224ff` has only `transcript.md`; `ROADMAP.md` lists them as planned). Governors twice asked "how much
time/cost was this?" (Rancho Maranta/SunMint impact-map thread) and only a *lower bound* from git/PR merge
timestamps could be reconstructed — no per-call token or cost record exists. `scripts/append_usage.py`,
`scripts/rebuild_indexes.py`, `scripts/summarize_usage.py` are also absent (planned). **To fix:** implement
the `usage.jsonl` writer hooked into the autopilot service's tool-call pipeline (provider, model, tokens,
est_usd, timestamp), `meta.json` (session start/end, governor, thread_id), `messages.jsonl` (structured
turns), and a `summarize_usage.py` CLI for governor-facing cost answers. Backfill only future sessions;
don't rewrite existing transcripts.
