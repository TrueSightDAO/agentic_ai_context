...
### krake_ror disk-full durability: logrotate conf is host-local, lost on ASG recycle
**Filed 2026-09-06. Owner: unclaimed.** 2026-09-06 Bugsnag alert `[krake_ror] Errno::ENOSPC in krakes#check` (getdata.io/community/data-sources/check): `/home/ubuntu/krake_ror/log/production.log` (no logrotate config existed) filled the 7.8G root volume; Puma held a **3.98 GB deleted log inode** open (du/df gap ~3.6G), unreclaimable until process restart. **Immediate fix done on host** (`i-085896f3427372e0a`, 54.205.127.43): `sudo service krake_ror restart` → `/` 100% → 53%; wrote `/etc/logrotate.d/krake_ror` (daily + size 200M, rotate 5, copytruncate, delaycompress, `su ubuntu ubuntu`), force-run verified. **BUT the logrotate conf is host-local** — launch template `lt-085100be44b6079cc` v3 has **no user-data / BlockDeviceMappings**, so any future ASG recycle (the ASG already replaced the box once on 2026-09-02 after an earlier ENOSPC) boots a fresh 8G-root instance with **no logrotate** and the bug returns in ~3–4 days. **To do (durable):** (1) add user-data bootstrap to the launch template that writes `/etc/logrotate.d/krake_ror` on first boot (or bake into a custom AMI); (2) consider growing root volume to ≥20G (log grows ~13 KB/s ≈ 1.1 GB/day of normal request logging); (3) optionally add a monit/alert on `df /` ≥ 85%.
