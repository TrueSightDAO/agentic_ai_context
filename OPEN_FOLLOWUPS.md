...
**Same race hit a CODE repo 2026-09-06 (thread-19965):** my Santa Ana map-fix PR (agroverse_shop_beta #300, sha ad9c54e6, 01:05Z — removed a stray `;` breaking the farm page's inline map script) was **silently reverted by a concurrent "Track A rollout" commit (2457f948, 01:18Z, "add Verified & Traceable FSVP blocks to 12 farm pages")** that rewrote the same file from a pre-fix base and merged afterward. Both beta and prod main ended on the buggy blob 585545ea; the live map stayed broken. **Lesson (code-repo variant):** (1) on actively-edited farm pages, check the file's last-commit age before pushing, and re-check it after merge — a newer unrelated commit may have just clobbered you; (2) **verify the post-merge blob via cache-busted contents API + node --check, never trust raw CDN or a same-name cross-repo compare** (`main...TrueSightDAO:main` on the prod fork compares the fork to ITSELF, not to beta — useless); (3) `sync_beta_to_prod` after the fix is verified on beta main. Fix re-applied cleanly as agroverse_shop_beta #304 (merged d9aca714e7) → synced to prod 02:29Z → live verified. Consider a check-commit-age-before-push rule for farm pages.
