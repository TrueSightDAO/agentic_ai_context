...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SOPHIA_LIVE_PROGRESS_PLAN.md` | Live Progress Introspection (report what the executing turn is doing when Gary asks mid-turn, instead of a canned queued ack) | 2026-06-11 | **completed** | PR1 (#160) + PR2 (#161) merged and deployed. Step G (idle-before-redeploy + thread dashboard) moved to Governance & Vault plan (thread 2744). | 2026-06-11 |
...
When Sophia starts, completes, or resumes a handoff, she updates the relevant row's
