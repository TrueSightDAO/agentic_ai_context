...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **PR2 done** — `GET /dao/review_queue` endpoint merged to `dao_protocol` (PR #125). Cursor-based pagination, 30s cache, GitHub Contents API. **ONE PR PER TURN:** next turn runs **PR3** (CONTRIBUTION REVIEW EVENT handler + cache deletion). Then PR4 (GAS write-back script) → PR5 (DApp review queue page) → PR6 (dao_client module) → PR7 (beta deploy + UAT) → PR8 (promote to prod). RESUME HERE = PR3. | 2026-06-18 |
