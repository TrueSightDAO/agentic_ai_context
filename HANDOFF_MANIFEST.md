...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SOPHIA_FOLLOWUP_MONITOR_PLAN.md` | Durable Follow-up Monitor (thread-bound, multi-day; gmail_reply + elapsed_days; on strike spins a Sophia turn in the thread) | 2026-06-11 | blocked | Plan merged; **awaiting trigger → Sophia parks GO-ready (thread TBD)**; on GO runs PR1→PR4 (RESUME HERE = PR1 `app/followups.py`); **own-repo gate: opens PRs only, NEVER self-merges**; UAT U1–U9 is the completion gate | 2026-06-11 |
