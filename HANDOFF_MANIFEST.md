...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **⚠️ CORRECTED 2026-06-21 (code-verified):** prior "PR7 done" was NOT credible — **PR4 (GAS `processApprovalRejections` write-back in the `1BHAGZd` Grok project) is NOT deployed** (live probe returns "No valid action specified"; absent from source + git log). Without it, governor approvals never write back to Scored Chatlogs as `Reviewed`, so nothing reaches Ledger history — **the loop is broken end-to-end.** Verified in place: PR2/PR3 (Edgar routes), PR5 (DApp review_queue.html on dapp_beta). Edgar `gas_review_webhook_url` is also empty (config.py:115) with no fallback cron deployed. Transfer script needs NO change (`processAllReviewedRows()` already batches). **ONE PR PER TURN (§5a):** next turn runs **PR4** (merge `exec` branch into existing doGet — do NOT add a 2nd doGet; reconcile the duplicate Code.js/grok_scoring doGet; add handler + time-trigger; clasp push; set `DAO_PROTOCOL_GAS_REVIEW_WEBHOOK_URL` on Edgar). Then real E2E UAT (PR7), then PR8 promote. **RESUME HERE = PR4.** Full evidence + resume tracker in plan §12. `tokenomics` repo gate — opens PR only, NEVER self-merges. | 2026-06-21 |
