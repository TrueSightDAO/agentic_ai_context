...
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **2026-06-22: PR4 DEPLOYED.** Handler merged (tokenomics #367); duplicate-`doGet` deploy-blocker fixed (#368); then `clasp push`ed 1BHAGZd with the gary account (also repaired a drifted @HEAD: removed orphan Code.js, restored Credentials.js, single doGet). Created **anonymous versioned deployment @2** `AKfycbzati5N6aT1slb5C8SAIfs11avrAg_8Wf_ecXXMmoUp0K6I3-TnDwIlv1Cth4IHOQMq` — `?exec=processApprovalRejections` **verified live** → `{"status":"ok","processed":0,"skipped":0}` (Telegram @1 deployment untouched). **PR4-WIRE DONE** (Edgar webhook env set on `dao_protocol_nelanco`, write-back path live). **BUT UAT BLOCKED (2026-06-22):** the `review_queue.html`↔Edgar contract is broken — page calls `/api/v1/dao/review_queue` (404; real is `/dao/review_queue`), `/dao/contributors` (404, no such route), and `POST /dao/verify_identity` (404, no such route). `verify_identity` 404 → `isGovernor` always false → **no Approve buttons render** → nothing to UAT. Also queue is empty (PR1 cache generator hasn't run) and `beta.edgar` has no review routes (only prod wired). **RESUME HERE = PR-INTEGRATION** (fix page `/api/v1`; implement or repoint verify_identity + contributors to the existing governor registry / contributor cache; seed a cache file), THEN PR7 UAT, THEN PR8. Full evidence in plan §12.3. | 2026-06-22 |
