...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **PR3 done** — `POST /dao/submit_contribution_review` merged to `dao_protocol` (PR #126). Governor/Sentinel auth, cache deletion, GAS callback. **ONE PR PER TURN:** next turn runs **PR4** (GAS doGet for processApprovalRejections). Then PR5 (DApp review queue page) → PR6 (dao_client module) → PR7 (beta deploy + UAT) → PR8 (promote to prod). RESUME HERE = PR4. | 2026-06-18 |
