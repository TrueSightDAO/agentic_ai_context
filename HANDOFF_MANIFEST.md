...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **PR6 done** — `report_contribution_review.py` + CLI entry point merged to `dao_protocol`. `truesight-dao-report-contribution-review` CLI with Approve/Reject/Skip actions, RSA signing, dry-run, beta/prod support. **ONE PR PER TURN:** next turn runs **PR7** (beta deploy + UAT). Then PR8 (promote to prod). RESUME HERE = PR7. | 2026-06-18 |
