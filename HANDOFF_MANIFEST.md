...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **PR5 done** — `review_queue.html` merged to `dapp_beta` (PR #52). Infinite scroll, three-action panel, contributor resolution dropdown, RSA signing, beta/prod auto-detect. **ONE PR PER TURN:** next turn runs **PR6** (dao_client `report_contribution_review` module). Then PR7 (beta deploy + UAT) → PR8 (promote to prod). RESUME HERE = PR6. | 2026-06-18 |
