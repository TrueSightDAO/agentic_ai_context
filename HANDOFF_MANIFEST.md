...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | parked GO-ready | Parked GO-ready in **thread 7191** ([topic](https://t.me/c/3919341801/7191)). **ONE PR PER TURN:** on GO run **PR1 ONLY then STOP** (GitHub Action cache generator for `treasury-cache`). Then PR2 (Edgar review queue endpoint) → PR3 (CONTRIBUTION REVIEW EVENT handler + cache deletion) → PR4 (GAS write-back script) → PR5 (DApp review queue page) → PR6 (beta deploy + UAT) → PR7 (promote to prod). `treasury-cache` API-only DATA repo — single-file writes via `upload_file_to_github`; `dapp_beta` code repo — branch → PR; `tokenomics` GAS — branch → PR; RESUME HERE = PR1. | 2026-06-18 |
