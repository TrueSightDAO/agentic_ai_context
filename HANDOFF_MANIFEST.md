...
|-----------|---------------|--------------|--------|---------------------|---------------------|
| `SCORING_REVIEW_QUEUE_PLAN.md` | Scoring Review Queue (cache generator, Edgar endpoint, DApp review UI) | 2026-06-18 | in progress | **PR1 done** — pushed `generate_review_cache.yml` workflow + `generate_review_cache.py` script to `treasury-cache` main. **ONE PR PER TURN:** next turn runs **PR2** (Edgar `GET /dao/review_queue` endpoint). Then PR3 (CONTRIBUTION REVIEW EVENT handler + cache deletion) → PR4 (GAS write-back script) → PR5 (DApp review queue page) → PR6 (dao_client module) → PR7 (beta deploy + UAT) → PR8 (promote to prod). RESUME HERE = PR2. | 2026-06-18 |
