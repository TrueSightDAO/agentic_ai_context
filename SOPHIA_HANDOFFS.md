...
|-----------|---------------|----------------|-------------------|--------------|--------|
| `MEMBERS_PAGE_SINGLE_SOURCE_PLAN.md` | Members page reliability + single-source consolidation | _(register on trigger)_ | _TBD_ | 2026-07-01 | draft — awaiting trigger + GO; §5a one PR per turn; RESUME HERE = **PR1** (lineage-credentials cron 6h→2h); PR2 gate = `LINEAGE_DISPATCH_PAT` secret; PR4 gate = beta→prod human promotion; cross-repo — opens PRs only, never self-merges |
