...
|-----------|---------------|----------------|-------------------|--------------|--------|
| `sentiment_importer/LARGE_SPIKE_INDEX_EXECUTION_ROADMAP.md` (repo: TrueSightDAO/sentiment_importer) | Large Spike Index (`/large_spikes`) — bullish/momentum mirror of the sell-off index | _(register on GO)_ | — | 2026-07-02 | draft — awaiting trigger + GO; roadmap merged to sentiment_importer `master` (PR #1098); §5a one PR per turn; RESUME HERE = **PR1** (migration `add large_spike` + `mark_large_spikes`, `gate: eyeball predicate`); own-repo — opens PRs only, never self-merges |
