...
|------|---------|-----------|-------|-----------|------------------------|--------|
| 2026-06-11 | Durable Follow-up Monitor (thread-bound, multi-day follow-ups; `gmail_reply` + `elapsed_days`; on strike spins a Sophia turn in the originating thread) — **on Sophia's OWN codebase** | `SOPHIA_FOLLOWUP_MONITOR_PLAN.md` | _pending — Sophia to create on trigger_ | _TBD_ | _TBD_ | **handed off — awaiting trigger**; on trigger Sophia creates topic `Follow-up Monitor`, parks GO-ready (RESUME HERE = PR1 `app/followups.py`); **opens PRs only, NEVER self-merges own-repo PRs** (human merges); UAT U1–U9 = completion gate |
