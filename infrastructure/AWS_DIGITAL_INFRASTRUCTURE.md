...
| **krake_ror** (ASG-managed) | `i-085896f3427372e0a` | t2.micro | running | 172.31.17.3 | 54.205.127.43 | Krake Rails backend (getdata.io). Behind ALB `krake-ror-1` (TG `krake-ror-1`:3002, ELB health). ASG `krake_ror` Min=Max=Desired=1, launch template `lt-085100be44b6079cc` v3 (GETDATA_key_pair; no BlockDeviceMappings → 8G root). **Recycled 2026-09-02 06:17** — previous `i-0df7a9e513dc537a6` (18.205.20.43) was replaced after an ENOSPC disk-full (see OPEN_FOLLOWUPS.md). SSH needs `PubkeyAcceptedKeyTypes=+ssh-rsa`. |
