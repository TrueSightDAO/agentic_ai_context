...
| **krake_ror** (ASG-managed) | `i-069771c1f79288216` | t2.micro | running | 172.31.24.138 | 98.81.159.70 | Krake Rails backend (getdata.io). Behind ALB `krake-ror-1` (TG `krake-ror-1`:3002, ELB health). ASG `krake_ror` Min=1 Max=2 Desired=1, launch template `lt-085100be44b6079cc` **v5** ($Default) → custom AMI **`ami-0adfbc216d627e058`** (`krake_ror_20260906b`; GETDATA_key_pair; no BlockDeviceMappings → 8G root) with logrotate baked in (v4 / `ami-0933e020a3e613189` `krake_ror_20260906` = rollback). v5 adds `/etc/logrotate.d/krake-ror-upstart` (daily + size 100M, rotate 7, copytruncate) for `/var/log/upstart/krake_ror-0.log` — the 202M unbounded upstart log found 2026-09-06 (~95 MB/hr growth; NOT covered by `/etc/logrotate.d/krake_ror`, which handles production.log only). **Rolled 2026-09-06** (zero-downtime ASG swap: scale to 2 → validate → drain → terminate) — replaced `i-085896f3427372e0a` (54.205.127.43); ENOSPC durable fix, see OPEN_FOLLOWUPS.md. Live box already has both rules (upstart rule force-rotated 2026-09-06 02:30 UTC). SSH needs `PubkeyAcceptedKeyTypes=+ssh-rsa`. |
...
| **krake_data** | `i-07c76510b231d787f` | t3.medium | running | 172.31.19.2 | 52.5.179.48 | Krake data processing. Root 7.7G — hit **100% / ENOSPC 2026-09-06** (46 old kernel sets never purged, 316d uptime; rsyslog ENOSPC since 2026-06-04); remediated → 30% / 5.4G free. Postgres 9.5 `dev_panel` (serves krake_ror + krake_sk_consolidated) on **separate 50G vol** `/krake_data_cache` (`/dev/nvme1n1`, 51%). Remote df alert from autopilot cron via consolidated `df-alert-fleet.sh` (30-min, ≥85/93%; supersedes df-alert-remote-krakedata.sh). See OPEN_FOLLOWUPS Recently shipped. |
