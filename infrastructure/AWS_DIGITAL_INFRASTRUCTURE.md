...
| **krake_ror** (ASG-managed) | `i-0f7f3490dc465136b` | t2.micro | running | 172.31.28.224 | 54.224.186.212 | Krake Rails backend (getdata.io). Behind ALB `krake-ror-1` (TG `krake-ror-1`:3002, ELB health). ASG `krake_ror` Min=1 Max=2 Desired=1, launch template `lt-085100be44b6079cc` **v4** ($Default) → custom AMI **`ami-0933e020a3e613189`** (`krake_ror_20260906`; GETDATA_key_pair; no BlockDeviceMappings → 8G root) with logrotate baked in. **Rolled 2026-09-06** (zero-downtime ASG swap: scale to 2 → validate → drain → terminate) — replaced `i-085896f3427372e0a` (54.205.127.43); ENOSPC durable fix, see OPEN_FOLLOWUPS.md. SSH needs `PubkeyAcceptedKeyTypes=+ssh-rsa`. |
