...
| **tokenomics** | `tokenomics/` | DAO tokenomics: Apps Scripts, Python scripts, Raydium market making, schema/API. **GAS:** local **`clasp_mirrors/<scriptId>/`** for `clasp push`/`pull`; git keeps **`.clasp.json`** + checklist/manifest only — **mirror `*.js` / `appsscript.json` gitignored** (`clasp pull` after clone). **`google_app_scripts/`** = reference `.gs` layout. **Agroverse store inventory web app** **`update_store_inventory`:** mirror **`clasp_mirrors/1P0Mg33i_dD9x9IeoHYvtKrf0xFcmUznpqAswyC_KXR3VJZu-0C-UOP0v/`** (deploy URL in **`Code.js`** header); may publish snapshots to **`agroverse-inventory`** using Script property **`AGROVERSE_INVENTORY_GIT_REPO_UPDATE_PAT`**. **Note:** LATOKEN on hold. Wix deprecated. **Holistic Hit List HITL API:** `google_app_scripts/holistic_hit_list_store_history/store_interaction_history_api.gs` (URL in header). | GAS, Python, TypeScript | `clasp_mirrors/`, `google_app_scripts/` (reference), `python_scripts/`, `API.md`, `SCHEMA.md` | QR_CODE_REPOSITORY_TOKEN, GDRIVE_KEY, WIX_* (deprecated), GITHUB_*; GAS **Script properties** for **`update_store_inventory`** (see **`API_CREDENTIALS_DOCUMENTATION.md`**) | [TrueSightDAO/tokenomics](https://github.com/TrueSightDAO/tokenomics) |
| **agroverse_shop** | `agroverse_shop/` | Agroverse e‑commerce (Wix migration): cart, Stripe, blog, farm/shipment pages. `agl15/` redirects to AGL15 Google Sheet ledger. **agroverse.shop** deployed from **agroverse_shop_prod**. **Store inventory:** **`js/inventory-service.js`** reads **`raw.githubusercontent.com/TrueSightDAO/agroverse-inventory/main/store-inventory.json`** first (uses **`inventory`** map), falls back to GAS **`update_store_inventory`** **`getInventory`**. **Videos from `~/Downloads` → blog/YouTube:** see **`agentic_ai_context/DOWNLOADS_MEDIA_TO_AGROVERSE.md`** (`analyze_incoming_videos.py`, `youtube_batch_incoming.py`, `generate_video_transcript_blog_posts.py`, `youtube_update_video_titles.py`, `youtube_oauth_reauthorize.py`, `grok_transcript_polish.py`). **Blog listing (`blog/index.html`):** Card thumbnails MUST use **`assets/images/blog/listing-640w/{post-slug}.jpg`** (max edge 640px JPEG ~quality 80), generated from **`scripts/sync_blog_listing_thumbnails.py`**. **Social previews:** After video posts or listing changes, run **`scripts/sync_post_open_graph_images.py`** so **`og:image` / `twitter:image`** match the listing card and include dimensions; use **`AGROVERSE_PUBLIC_ORIGIN`** when the deploy host differs (e.g. beta vs prod) so crawlers get HTTP 200 on images. **Content rule:** Prefer each post’s first real in-body `/assets/images/...` image (normalize `../../assets/`); skip logo; ignore generic `og:image` when it is only `partners/cic/cacao-tasting-wheel.jpg`. Text-only posts: curated distinct farm/partner/`assets/images/blog/bahia-photo-library/` shots. Enforce **unique** thumbnail URL and **unique** file hash across the grid; replace duplicates using `bahia-photo-library` or other repo assets. **Cursor rule:** `agroverse_shop/.cursor/rules/blog-listing-images.mdc`. **Lab report translation:** See `LAB_REPORT_TRANSLATION.md` when adding English summaries to shipment pages (AGL4, AGL8, etc.). **Physical product / packaging specs:** Ground truth for copy, colors, QR patterns — see **`PRODUCT_DEVELOPMENT_SPECS.md`** (Sheets workflow; service account `google-service-account.json` for API). **Important:** See `docs/PRODUCT_CREATION_CHECKLIST.md` when creating new products to prevent Merchant Center URL mismatches. **⚠️ SETUP REQUIRED:** See `docs/SECURITY.md` — requires credential files: `google-service-account.json`, `scripts/youtube_credentials.json`, `scripts/youtube_token.json` (NEVER commit these; prompt user during setup). | Static HTML/JS, Google Apps Scripts, Node (tests) | `scripts/`, `js/inventory-service.js`, blog in `post/`, `shipments/`, `agl15/`, `scripts/sync_blog_listing_thumbnails.py`, `scripts/sync_post_open_graph_images.py`, `scripts/translate_lab_report.py`, `tests/README.md`, `playwright.config.ts`, `.github/workflows/visual-consistency.yml`, `docs/PRODUCT_CREATION_CHECKLIST.md`, `docs/SECURITY.md` | NAMECHEAP_*, GOOGLE_API_KEY, AWS_* | [TrueSightDAO/agroverse_shop_beta](https://github.com/TrueSightDAO/agroverse_shop_beta) · Prod: [agroverse_shop_prod](https://github.com/TrueSightDAO/agroverse_shop_prod) |
| **agroverse-inventory** | `agroverse-inventory/` (optional clone) | **Public snapshot repo** for Agroverse **store inventory**: **`main`** / **`store-inventory.json`** (shape `{ generatedAt, source, inventory: { sku: n } }`). **Read URL:** `https://raw.githubusercontent.com/TrueSightDAO/agroverse-inventory/main/store-inventory.json`. **Writers:** GAS **`update_store_inventory`** (tokenomics **`clasp_mirrors/1P0Mg33i…`**) via Contents API + PAT **`AGROVERSE_INVENTORY_GIT_REPO_UPDATE_PAT`** (Script properties only). **Readers:** **`agroverse_shop`** **`js/inventory-service.js`** (primary); GAS **`getInventory`** fallback. **Edgar:** **`AgroverseInventorySnapshotPublishWorker`**, ledger **`WebhookTriggerWorker`** chain — see **`API_CREDENTIALS_DOCUMENTATION.md`**. | Git (JSON + README) | **`store-inventory.json`**, **`README.md`** | GAS Script properties; Edgar **`AGROVERSE_*`** env (see docs) | [TrueSightDAO/agroverse-inventory](https://github.com/TrueSightDAO/agroverse-inventory) |
