...
### PR5 — publisher + systemd timer (2026-09-13)
- `farm-media-daemon #27` merged: `farm_media_publisher.py` (idempotent reconcile, `--inbox`/`--collection`), `systemd/farm-media-publisher.{service,timer}`. `#28` merged: Option A image gating.
- **Timer installed on the box** (`/etc/systemd/system/`), `systemctl enable --now farm-media-publisher.timer`, `is-active = active`, next fire +15 min. First run: exit 0 (`status=0/SUCCESS`), 7.2s CPU.
- First run reconciled **16 collections → 15 new `galleries/*.json`** in `farm_media_manifests`; `cacau-na-veia-pacaje: unchanged` → idempotency proven.
