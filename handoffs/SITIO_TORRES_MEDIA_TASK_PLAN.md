...
> (`handoffs/CACAU_NA_VEIA_MEDIA_TASK_PLAN.md`). This is the **loc3 "Sítio 2"** visit
> (2026-09-09), the sibling of the loc1 Cacau na Veia walk.
...
## Goal
1. Confirm the plot for https://agroverse.shop/farms/sitio-torres-pacaja-para/ is durably
...
1. **Farm identity (governor-confirmed, thread 24442):** **Sítio Torres** — recorded in
   records as *"Sítio 2"* (a **propriedade/plot**, not its own separately-named farm).
   Wi-Fi SSID on site = `SITIO TORRES`. Region **Pacajá, Pará**. 4.5 ha (claimed).
   Cacao **TRUE/hybrid** ("verdadeiro, híbrido") + a small clonal block; plot ages 2/4/5 yr
   plus a **>20 yr** block; shade tree = **cumaru** (native Amazon). Sampled **~120 kg**;
   target first lot **600 kg**.
2. **Owner/producer:** **Alexandre** — a **CoopCao director-coordinator** (the coop has
   8 registered properties). People named in media are DATA, never ownership proof.
3. **Registry overlap FIRST (the Bom Sucesso trap) — ✅ ALREADY REGISTERED, not a trap.**
   Searched `sunmint/plots/index.geojson` for the loc3 GPS bbox
   `(-3.54020, -3.52880, -51.14554, -51.14330)`: **exactly 1 overlapping feature** —
   `plot_id: N-06-66`, `farm_id: sitio-torres-pacaja-para`,
   `name: "Sítio Torres (Pacajá) Plot 1 (cacao enrichment)"`, bbox **identical** to the
   walk bbox. Nearest-other centroid is ~111 m away. → **PR3 is verify-only**;
   do **not** add a second plot.
4. **Site code `N-06-66` — ⚠️ OUTSTANDING GATE (open follow-up).** The code was read as
   **N-06-66** from a phone-translator screenshot (`loc3/IMG_9694.PNG`: *"O código dele é
   N0666"*), but `CEPOTX_SITE_CODE_REGISTRY.md` lists the **COOPCAO** family as
   **N-06-02 …** — i.e. **N-06-66 sits outside the observed roster range**. The plot row
   *exists* in the registry; whether `N-06-66` is the **correct** issued code still needs
   **CEPOTX registry confirmation**. Do **not** treat it as re-derived; if the registry
   disconfirms, correct via a `[PLOT INVALIDATION EVENT]` + re-register.
5. **⚠️ Naming mismatch to resolve:** the work dir is `/media/pacaje_work/loc3/` but the
   source zip is `20260909_pacaje_location_2_nursery.zip`. Confirm `loc3` ≡ the "location 2"
   zip before archiving (a loc2/loc3 mixup would attach the wrong media to a farm).
6. **Claimed vs walked extent (flag):** claimed **4.5 ha**; the walk hull is **19.14 ha**
   (the walk covered nursery + mature blocks, not just the plot). Boundary authority stays
   **approx** — needs CAR/INCRA or a boundary walk to settle.

## Source media
- Dir on box: `/media/pacaje_work/loc3/` (unzipped from
  `/media/upload_zips/20260909_pacaje_location_2_nursery.zip`, 601 271 291 B).
- **38 HEIC + 35 MOV + 8 PNG = 81 files**, **2.1 GB**.
- **73 EXIF-bearing (HEIC+MOV); 72 geotagged (98.6%)**, 0 duplicates, **60 unique points**.
...
- Extent: lat `-3.54020 … -3.52880`, lon `-51.14554 … -51.14330` → convex hull
  **~19.14 ha** (10 vertices), centroid **-3.534869, -51.144765**.
- **8 PNGs are screenshots, not photos** (`IMG_9631/9637/9681/9690/9691/9694/9695/9698`) —
  `IMG_9694` is the site-code translator shot. Archive as *screenshots*, keep them **out**
  of the photo gallery.
- **Transcripts: DONE.** `/media/pacaje_work/loc3_tx/MAP/` = **35 `.json` + 35 `.vtt`**
  (faster-whisper, pt; MAP §6b) = one pair per MOV. Audio at
  `/media/pacaje_work/loc3_tx/audio/` (35 `.wav`); index `/media/pacaje_work/loc3_tx/transcripts.json`.
...
- **PR3** SunMint Plots **sheet row** verify for `N-06-66` (+ regenerate `sunmint/plots/index.geojson`
  only if the sheet drift implies it) — **expected NO-OP on the geojson**
- **PR4** agroverse_shop_beta — **expected NO-OP**: page + `media.json` gallery already live
  (only revisit if the gallery needs the newly-derived JPEGs)
...
```json
{"plot_id":"N-06-66","farm_id":"sitio-torres-pacaja-para","name":"Sítio Torres (Pacajá) Plot 1 (cacao enrichment)","status":"proposed","boundary_authority":"approx","owner":"Alexandre (CoopCao director-coordinator), CoopCao","region":"Pacajá, Pará"}
```
Registered bbox in `sunmint/plots/index.geojson` = `(-3.5402, -3.5288, -51.145542, -51.1433)`,
i.e. **exactly** the 72-point walk hull below → no new plot, no edit expected.

Walk ring (lng,lat, convex hull of 60 unique points — for the audit trail):
...
2. Videos (35 MOV) → MOV→MP4 transcode (ffmpeg, GPS re-inject via exiftool — VERIFY
   `Keys:GPSCoordinates` after) → transcription **already DONE** (commit the existing 35 pairs)
   → inbox sidecars (daemon schema + `creation_date` + `transcription`) → daemon uploads
   ~6/day → yt_ids backfilled.
3. Manifest commit (PR2) v2 schema: sha256/GPS/duration/objects[]/creation_date/transcription/
   transcription_status/yt_id.
4. **Config trap (from thread 24440):** set `extensions` **explicitly** in the archive-root /
   inbox config entry — a root **without** an `extensions` key silently defaults to MOV-only.
   Keep **PNG screenshots out of S3/photo dirs** unless deliberately archived.
...
- Two-way link: farm page → map must be a real
  `<a href="https://truesight.me/sunmint.html?plot=N-06-66">`; map → farm page needs no
  FARM_SLUG edit while `farm_id` == page slug.
- **Site-code gate:** resolve the `N-06-66` registry question (Step Zero §4) before treating the
  code as published truth.
...
Open questions for the governor: (a) confirm `N-06-66` is the issued CEPOTX code;
(b) confirm `loc3` ≡ the `..._location_2_nursery.zip` source.
