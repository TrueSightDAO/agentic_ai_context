...
- **2026-09-17** — **Cross-reference (separate plan):** the **CRF × SunMint tree-submission cohort** workstream is its own plan, `plans/CRF_ANAPU_SUNMINT_COHORT_PROPOSAL.md`. PR1–PR5 shipped 2026-09-16/17 — the SunMint app is vendored onto `cfr.truesight.me` `gh-pages` (PR2), `crf-anapu` carries `program_modes: ["cohort_credentialing", "sunmint_cohort"]`, and `members.html` gained SunMint activity badges + itemized click-through (PR5). **PR6** (first live `lineage-credentials` sync — the plan's only `gate: human`) is **blocked**: the PR3 sync dry-run reads 168 `tree_planting` / 2 `tree_growth_monitoring` / 19 `farm_boundary_evidence_event` source events but **0 attributable** to `crf-anapu` — no real `cfr.truesight.me`-sourced submission has landed yet. This media plan's own unit (the gallery UAT gate) is unaffected and still open.
