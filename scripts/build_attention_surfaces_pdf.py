#!/usr/bin/env python3
"""Generate the shareable ATTENTION_SURFACES.pdf from ATTENTION_SURFACES.md.

Usage:  python3 scripts/build_attention_surfaces_pdf.py
Deps:   pip install markdown weasyprint

Landscape A4 so the ten-surface table reads comfortably. Trigram glyphs
(STIX-range symbols) render via the PingFang SC font fallback on macOS /
...
SRC = ROOT / "ATTENTION_SURFACES.md"
OUT = ROOT / "ATTENTION_SURFACES.pdf"
...
@page {
    ...
    margin: 1.8cm 1.6cm;
    @bottom-center {
        content: "TrueSight DAO · Attention Surfaces · page " counter(page) " of " counter(pages);
        font-size: 8pt;
        color: #8a7b6b;
    }
}
...
h1 { font-size: 17pt; color: #4a2f1b; border-bottom: 3px solid #c8801a; padding-bottom: 5pt; }
