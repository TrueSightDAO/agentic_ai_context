...
def check_index(manifest_text: str, index_path: Path) -> ValidationResult:
    ...
    ``handoffs/index.json`` is generated by ``scripts/build_handoff_index.py``; this is
    the drift gate that fails CI when the committed index no longer matches the manifest.
    The volatile ``generated_at`` field is ignored so a rebuild elsewhere does not read as
    drift. The manifest is the single source of truth — the index is always regenerated
    from it, never reconciled by hand.
    """
    result = ValidationResult()
    if not index_path.exists():
        result.errors.append(
            f"index not found: {index_path} — generate it with scripts/build_handoff_index.py"
        )
        return result
    ...
    added = [p for p in fresh_plans if p not in committed_plans]
    removed = [p for p in committed_plans if p not in fresh_plans]
    if added:
        result.errors.append(f"index.json is missing handoff(s): {added}")
    if removed:
        result.errors.append(f"index.json has stale handoff(s): {removed}")
    if not added and not removed and fresh.get("handoffs") != committed.get("handoffs"):
        result.errors.append(
            "index.json handoff rows differ from the manifest (a field changed) — "
            "regenerate with scripts/build_handoff_index.py"
        )

    for key in ("source_sha256", "handoff_count", "counts_by_state"):
        if fresh.get(key) != committed.get(key):
            result.errors.append(
                f"index.json {key} drifted (manifest {fresh.get(key)!r} != "
                f"index {committed.get(key)!r}) — regenerate with scripts/build_handoff_index.py"
            )
    return result
