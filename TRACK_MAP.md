...
flowchart TD
    ...
    T7[Brazil Export CNPJ<br/>CNAE / IE / NF-e]:::gate
    T8[Brazil → China Export<br/>GACC + CNPJ both needed]:::gate
    T7 --> T8
    T7 --> T10
    T7 --> T11
    T7 --> T12
    ...
    T13[Chocolate Subscription Delivery<br/>China subscribers]:::blocked
...
| **Dependencies** | None — parallel to [Brazil Export CNPJ](#brazil-export-cnpj) |
| **Blocks** | [Brazil → China Export](#brazil-to-china-export) — GACC is one of two parallel gates for China-bound cargo |

---

### Brazil Export CNPJ (CNAE / IE / NF-e) {#brazil-export-cnpj}
...
| **Blocks** | [Brazil → China Export](#brazil-to-china-export) (one of two parallel gates), [Chives Root Consignment](#chives-root-consignment), [Michael Johnson Consignment](#michael-johnson-consignment), [Kopi Bar Jul 10 Tasting](#kopi-bar-jul-10-tasting) |

> **Note:** USA-bound cargo (Chives Root, Michael Johnson, Kopi Bar) only needs the new CNPJ — the US import side is already live via TrueTech Inc (Delaware C-Corp, FDA-registered, Customs importer-of-record). China-bound cargo needs **both** the new CNPJ **and** GACC registration.

---

### Brazil → China Export (GACC + CNPJ) {#brazil-to-china-export}

| Field | Detail |
|-------|--------|
| **Status** | 🔴 Gate — two parallel prerequisites |
| **Owner** | Gary / Paloma / BoQiang / Matheus |
| **Goal** | Clear both gates before first commercial cargo can move from Brazil to China: (1) New export CNPJ with correct CNAE, and (2) GACC registration (3–6 months). Both must be complete before import customs clearance. |
| **Dependencies** | 🔴 **Blocked by** [Brazil Export CNPJ](#brazil-export-cnpj) **and** [GACC / Brazil Compliance](#gacc-brazil-compliance) — both must clear |
| **Blocks** | [China / Aora Events](#china-aora-events), [Chocolate Subscription Delivery](#chocolate-subscription-delivery) (China subscribers) |

> **July event samples — separate fast path:** BoQiang has arranged an alternative channel for July event samples. Various cacao raw material samples (sensory teaching aids) can be sent via courier to Dongguan Airport. Required docs: plant inspection certificates, certificates of origin, organic qualifications, formal invoices, packing lists, cooperative Brazilian CNPJ registration docs. BoQiang will arrange freight forwarder to handle customs clearance. This path does **not** require the full export CNPJ or GACC — it uses the cooperative's existing docs.
...
| **Dependencies** | 🔴 **Blocked by** [Brazil → China Export](#brazil-to-china-export) — China subscribers need both CNPJ + GACC. USA subscribers only need the new CNPJ. |
...
| **Dependencies** | 🔴 **Blocked by** [Brazil → China Export](#brazil-to-china-export) — bars need both CNPJ + GACC for commercial cargo. July event samples may use the separate fast path via BoQiang (cooperative docs + courier to Dongguan). |
...
| **Dependencies** | 🔴 **Blocked by** [Brazil Export CNPJ](#brazil-export-cnpj) — USA-bound, only needs the new CNPJ (US import side already live via TrueTech Inc) |
...
| **Dependencies** | 🔴 **Blocked by** [Brazil Export CNPJ](#brazil-export-cnpj) — USA-bound, only needs the new CNPJ |
...
| GACC / Brazil Compliance | 🟡 Offline | Gary / Paloma / BoQiang | 3–6 month cycle | Brazil → China Export |
| Brazil Export CNPJ | 🔴 Gate | Matheus / Paloma / Gary | ~Jun 26 | USA cargo + Brazil → China Export |
| Brazil → China Export | 🔴 Gate | Gary / Paloma / BoQiang / Matheus | Both CNPJ + GACC | China cargo |
| Chocolate Subscription | 🟡 Blocked | Gary | — | CNPJ (USA) / CNPJ+GACC (China) |
| China / Aora Events | 🟡 Blocked | Gary / Liz / Jerri | Jul beta | Brazil → China Export |
| Chives Root Consignment | 🟡 Blocked | Chives Root / Gary | — | Brazil Export CNPJ |
| Michael Johnson Consignment | 🟡 Blocked | Michael Johnson / Gary | — | Brazil Export CNPJ |
| Kopi Bar Jul 10 Tasting | 🟡 Blocked | Nora Haron / Gary | Jul 10 | Brazil Export CNPJ |
