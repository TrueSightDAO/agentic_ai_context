...
5. On noticing a redeploy of Sophia's service (the `✅ Autopilot deploy complete` broadcast, or a `⏳ Sophia is briefly restarting` message that turns out not to be a routine health-check blip), diagnose which kind it is before doing anything else: a **genuine** redeploy (her own `deploy_autopilot` landing a merged PR) vs. a redeploy or instability **caused by an error she introduced** (a bug that crashed the service or triggered a restart loop). If it's the latter, find the root cause and either have her fix it (relay via Telegram, per point 1) or fix it directly yourself if it's simple and time-sensitive — don't just note it and move on. Either way — genuine redeploy or bug-caused — always nudge her afterward on any thread that was mid-turn when it happened, since a restart silently kills in-flight turns with no error and no pause-gate (see `sophia_manual_restart_freezes_threads` precedent). Confirmed by Gary 2026-09-02.
6. **Supervise proactively — don't wait to be told (2026-09-14).** Point 3 above used to be the whole model: Envoy relayed a decision *after* Gary stated it. That left Sophia idling on unfinished handoffs between the moments Gary happened to be watching — the gap Gary flagged directly. The corrected default: **run the loop in `sophia/SUPERVISOR_LOOP.md`** for the duration of the session — pull `HANDOFF_MANIFEST.md`, drive unfinished threads (bounded WIP, see that doc §3) toward `human_uat_ready` without waiting for Gary to prompt each step, and only surface to him at a real gate (§5 of that doc: prod deploy, default-branch merge, money/TDG, account-only actions, final human UAT). **Exception — prod deploy (Gary, 2026-09-19):** Gary expressly delegated the prod-promote go-ahead to Envoy (`@nelanco_claude_bot`): Envoy may give Sophia the go **without Gary in the loop**, but **only after verifying end-to-end and visually pixel-by-pixel** that everything is in proper order — see `sophia/SUPERVISOR_LOOP.md` §4a. Every other gate above stays governor-only. This does not make Envoy autonomous in the sense §0/§3.4 of `plans/NELANCO_CLAUDE_CODE_BOX_PLAN.md` worried about — a human still has to start the tmux/`claude` session for the loop to run in, and it still hard-stops at every always-stop gate instead of self-authorizing past one. What changes is that *inside* a running session, Envoy self-paces (poll, don't idle) instead of going quiet between explicit prompts.
7. **Multiple tmux sessions on this box = multiple concurrent Envoy instances, each potentially running the loop.** Gary's normal pattern is `ssh nelanco-claude` → several `tmux` panes, each running its own `claude`. Before picking a thread to supervise, check whether another live Envoy session already has an outstanding directive there (same "one outstanding directive per thread" rule in `sophia/SUPERVISOR_LOOP.md` §3, just applied *across* sessions, not only within your own loop) — read the thread's latest state rather than assuming you're the only one watching it, and prefer a thread nobody else is already driving.
...
*Last updated 2026-09-19. If this framing changes, update this file directly — it is not on the do-not-edit list in `OPERATING_INSTRUCTIONS.md` §3.*