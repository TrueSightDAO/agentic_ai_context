...
| **GOVERNANCE_SOURCES.md** | **Governance** — Whitepaper (now a static page: `truesight_me/whitepaper/index.html` → truesight.me/whitepaper/; **Google Doc deprecated**), proposals (GitHub TrueSightDAO/proposals, Realms). |
...
| **scripts/fetch_whitepaper.py** | **DEPRECATED** — pulled whitepaper content from the now-retired Google Docs. Live whitepaper is `truesight_me/whitepaper/index.html`; read/edit that. Kept for history. |
| **WHITEPAPER_SNAPSHOT.md** | **DEPRECATED capture** of the old Google Doc — does not match the live page. Read `truesight_me/whitepaper/index.html` instead. |
| **EDGAR_WHITEPAPER_SNAPSHOT.md** | **DEPRECATED capture** — read `truesight_me/edgar/whitepaper/index.html`. |
| **AGROVERSE_WHITEPAPER_SNAPSHOT.md** | **DEPRECATED capture** — read `truesight_me/agroverse/whitepaper/index.html`. |
| **SUNMINT_WHITEPAPER_SNAPSHOT.md** | **DEPRECATED capture** — read `truesight_me/sunmint/whitepaper/index.html`. |
...
| **REPACKAGING_SETTLEMENT_SOP.md** | **Repackaging settlement SOP** — How to finalize a repackaging batch: gather info, lookup SKU IDs from `Agroverse SKUs` tab, run `[REPACKAGING SETTLEMENT EVENT]` via CLI, verify offchain transactions + asset location + store-inventory.json. Includes SKU catalog reference, CLI template, verification checklist, and common failure modes. **Read when settling a repackaging batch** (after the `[REPACKAGING BATCH EVENT]` GAS has created Currency rows). |
