...
| **Infra & Tools** | jarvis, read_page, iching_oracle, point-of-sales-integrations, heierling-pos, **Cypher-Defense**, **truesight_autopilot**, **Bionpact** | Local LLM, browser extension, oracle app, POS integrations; **Cypher-Defense** — Web3 scam/phishing extension **and** AWS incident-response scripts + `docs/incidents/` (local `.env` for AWS keys; never commit); **truesight_autopilot** — unified AI service: governor chat via DApp (`chat.html`) + autonomous SRE/developer that monitors email/AWS/GCP health, diagnoses failures with DeepSeek-V3, and opens fix PRs (never auto-merges). Merged from `governor_chatbot_service` (deprecated). Deploys on dedicated EC2 separate from Perch (Rails); **Bionpact** — locked-down, private sibling instance of `truesight_autopilot` for the Ops team (Gary Teh + Elizabeth Wong) only. Same codebase, different `.env`: Telegram-only (no public HTTP surface), no DAO signing/AWS/Gmail/fleet-SSH credentials, reads all public TrueSightDAO repos but writes only to her own three private repos (`bionpact_agentic_ai_context`, `bionpact_autopilot_transcription`, `bionpact_attachments` — **not readable by Sophia or other agents**). Own EC2 box in Nelanco, VPC-restricted security group, no public inbound. See `PROJECT_INDEX.md` and `plans/BIONPACT_AUTOPILOT_PLAN.md`. |
...
- **Static sites**: truesight_me, agroverse_shop, dapp — often deployed to GitHub Pages or similar; design uses “Saffron Monk” / earthen palette where noted. **Slide decks generated by Sophia or any LLM MUST follow `SLIDE_DECK_STANDARD.md`** (Saffron Monk 16:9, wrap-safe text, clickable links; reusable renderer `truesight_autopilot/templates/slide_deck_template.py`).
...
3. Perch (Rails) dispatches `WebhookTriggerWorker` → GAS (`asset-receipt-ingest`) → Currencies row + offchain inventory leg + **"Asset Receipts"** audit tab
...
- **dapp** ↔ **holistic wellness Hit List (Sheets)**: `store_interaction_history.html` calls the read-only web app documented under **`tokenomics/google_app_scripts/holistic_hit_list_store_history/`** (Deployment URL in `store_interaction_history_api.gs`; DApp **`API_BASE_URL`** should match after redeploys). **Deploy changes** from the matching **`tokenomics/clasp_mirrors/<scriptId>/`** project (`clasp push`), not from the thematic folder alone. Spreadsheet: `1eiqZr3LW-qEI6Hmy0Vrur_8flbRwxwA7jXVrbUnHbvc`. Partner workflow: **`PARTNER_OUTREACH_PROTOCOL.md`**, **`HIT_LIST_CREDENTIALS.md`** (market_research). **Email-agent open/click tracking:** Perch **`/email_agent/open.gif`** and **`/email_agent/click`** (**TrueSightDAO/sentiment_importer**) update **Email Agent Drafts** columns **Open** / **Click through**; **go_to_market** scripts **`sync_email_agent_followup.py`**, **`regenerate_pending_email_agent_draft_tracking.py`**, and **`reconcile_email_agent_drafts_stale_sent.py`** keep Gmail, drafts tab, and **Email Agent Follow Up** aligned. For LLM subject/content tuning, **Email Agent Drafts** is the canonical engagement source — Follow Up L/M is frozen at send-time. See **`HIT_LIST_CREDENTIALS.md`** §"Performance analysis / LLM tuning — which sheet to use".
...
- **TrueChain** ↔ **tokenomics** / **Edgar**: Private blockchain for DAO/Agroverse audit trail. Mirror Service copies new rows from Google Sheets to TrueChain. Block explorer via Google Apps Script (not the DAO server). See `TRUECHAIN_README.md`, `TRUECHAIN_SETUP_AND_INTEGRATION.md`.
...
- **Governance**: this repo `GOVERNANCE_SOURCES.md` — Whitepaper (now a static page: `truesight_me/whitepaper/index.html` → truesight.me/whitepaper/; **Google Doc + `fetch_whitepaper.py` deprecated**), proposals (GitHub TrueSightDAO/proposals, Realms; browser for Realms).
...
- **Serialized QR bulk sales (post-minted QR codes):** `notes/claude_serialized_qr_sales_YYYY-MM-DD.md` (or newest matching) — one **`[SALES EVENT]`** per QR code (`Item` = QR code ID), **sufficient on its own** (downstream — QR Code Sales tab → offchain transactions → treasury cache — auto-depletes inventory), plus live GAS endpoint discovery (`?list_with_members=true`). **Do NOT add an `[INVENTORY MOVEMENT]` to "deplete" the `offchain asset location`** — that event only transfers inventory custody person-to-person, it does not deplete a location (Gary 2026-06-18). Reusable template: **`dao_client/examples/bulk_qr_sales_template.py`**. See also **PROJECT_INDEX.md** `dao_client` row.
...
**Rails hosts (do not conflate):** **`getdata.io`** is the production deploy for **`krake_ror`** ([KrakeIO/krake_ror](https://github.com/KrakeIO/krake_ror)) — the Krake **data-harvesting** Rails app. **`edgar.truesight.me`** serves **Edgar**, the standalone Python/FastAPI **`dao_protocol`** server ([TrueSightDAO/dao_protocol](https://github.com/TrueSightDAO/dao_protocol)) — DAO submission + dispatch, running on its own host. **`perch.truesight.me`** is the separate Rails **`sentiment_importer`** app ([TrueSightDAO/sentiment_importer](https://github.com/TrueSightDAO/sentiment_importer)). **Naming convention (2026-06-13):** **Perch** = the Rails `sentiment_importer` (DAO API, Sidekiq, Meta checkout, inventory snapshot workers). **Edgar** = the Python/FastAPI `dao_protocol` server (DAO submission + dispatch). `dao_protocol` and `sentiment_importer` are **independent standalone codebases on different hosts** — `sentiment_importer` is **no longer the source for Edgar** (it was, before the extraction). During the migration tail, the nginx in front of `edgar.truesight.me` (hosted on the Perch box) still proxies a few legacy paths to Rails, but the two are distinct codebases; do not assume APIs or env vars on one apply to the other.
...
| **getdata.io** | [KrakeIO/krake_ror](https://github.com/KrakeIO/krake_ror) | Krake Rails backend — **not** Perch / sentiment_importer |
| **edgar.truesight.me** | [TrueSightDAO/dao_protocol](https://github.com/TrueSightDAO/dao_protocol) | **Edgar** — standalone Python/FastAPI `dao_protocol` (DAO submission + dispatch), own host. Separate codebase from `sentiment_importer`; nginx (on the Perch box) still proxies a few legacy paths to Rails during the migration tail. |
| **perch.truesight.me** | [TrueSightDAO/sentiment_importer](https://github.com/TrueSightDAO/sentiment_importer) | **Perch** — Rails `sentiment_importer` (formerly called "Edgar"). DAO API, Sidekiq, Meta checkout helpers, `/agroverse_shop/shipping_rates`, `/ping`. |
