...
- **dapp** ↔ **holistic wellness Hit List (Sheets)**: `store_interaction_history.html` calls the read-only web app documented under **`tokenomics/google_app_scripts/holistic_hit_list_store_history/`** (Deployment URL in `store_interaction_history_api.gs`; DApp **`API_BASE_URL`** should match after redeploys). **Deploy changes** from the matching **`tokenomics/clasp_mirrors/<scriptId>/`** project (`clasp push`), not from the thematic folder alone. Spreadsheet: `1eiqZr3LW-qEI6Hmy0Vrur_8flbRwxwA7jXVrbUnHbvc`. Partner workflow: **`PARTNER_OUTREACH_PROTOCOL.md`**, **`HIT_LIST_CREDENTIALS.md`** (market_research). **Email-agent open/click tracking:** Edgar **`/email_agent/open.gif`** and **`/email_agent/click`** (**TrueSightDAO/sentiment_importer**) update **Email Agent Drafts** columns **Open** / **Click through**; **go_to_market** scripts **`sync_email_agent_followup.py`**, **`regenerate_pending_email_agent_draft_tracking.py`**, and **`reconcile_email_agent_drafts_stale_sent.py`** keep Gmail, drafts tab, and **Email Agent Follow Up** aligned (**`HIT_LIST_CREDENTIALS.md`**).
...
- **Edgar production log management (replaced aggressive log clearing):** `sentiment_importer/config/logrotate/sentiment_importer.conf` is installed on both hosts (`/etc/logrotate.d/sentiment_importer`) and rotates `log/*.log` **daily** with **7-day retention** + compression. This replaced `config/clear_log.sh` and `scripts/clear_logs.sh`, which previously ran via crontab and truncated logs every hour (destroying debug history). **Systemd journals** are now vacuumed weekly via crontab (`journalctl --vacuum-time=3d`) instead of `rm`. **`scripts/setup_logrotate.sh`** deploys the config and cleans up old crontabs. **Health check:** `curl https://edgar.truesight.me/ping` (should return `{"status":"success"}`), `ssh seni_ror "sudo systemctl is-active seni_ror"`, `ssh seni_sk "sudo systemctl is-active seni_sk"`. Monit dashboards: http://54.211.179.126:2812/seni_ror and http://3.83.175.151:2812/sidekiq.
...
- **Edgar production health / logrotate:** `sentiment_importer/config/logrotate/sentiment_importer.conf` + `scripts/setup_logrotate.sh`. Health check one-liner: `curl -s https://edgar.truesight.me/ping && ssh seni_ror "sudo systemctl is-active seni_ror" && ssh seni_sk "sudo systemctl is-active seni_sk"`.
