...
**Preview / “review recent progress” (no WhatsApp):** Run **`market_research/scripts/generate_beer_hall_preview.py`** from the **`market_research/`** clone (writes **`agentic_ai_context/previews/beer_hall_preview_latest.md`** by default). It gathers the same kind of evidence as a real Beer Hall post (**`OPENCLAW_WHATSAPP.md` § Gathering what merged** + **`list_recent_telegram_chat_logs_for_digest.py`**) but performs **no** `openclaw message send` and **no** sheet append — see **`OPENCLAW_WHATSAPP.md` § Preview digest — review recent progress**. The script **always emits the full Markdown on stdout** (see that section); when the user asks to **review recent progress**, assistants must surface that output in the **console / terminal transcript** (e.g. run the script via the agent shell so the user sees the digest without opening the file first). Still use **TrueSightDAO-only** GitHub URLs in any *Shipped* draft you add.

**LLM advisory materials (same review session):** When the operator asks to **review Beer Hall updates** or run the preview above, also run **`market_research/scripts/generate_advisory_snapshot.py`** so **`ADVISORY_SNAPSHOT.md`** and **`ecosystem_change_logs/advisory/snapshots/`** stay current for LLM advisors. Use **`--with-sheet-sales`** when **`market_research/google_credentials.json`** is available. **`OPENCLAW_WHATSAPP.md` § Closed loop** step **4** and **§ Preview digest** step **2**.

**Outbound digests:** That file also defines how to generate **Beer Hall** (full DAO / ops) and **Founder Haus AI prompt** (Web3 partner channel — **AI/agent subset**) WhatsApp summaries: required **OpenClaw × Cursor** attribution, **WhatsApp-safe** formatting, **Beer Hall** as **two sequential messages** — **Message 1** = attribution + **TLDR** only (plain language; **why it matters for the DAO**; no GitHub URLs); **Message 2** = *Shipped* bullets with **links** (PR/commit/Sheets) plus optional **Telegram Chat Logs** — then **sequential** sends (pause between); **Founder Haus** stays a **single** shorter message unless the operator says otherwise; **git push → PR → merge** before posting when code changed. **Inventory before writing:** **`OPENCLAW_WHATSAPP.md` § Gathering what merged** — poll **all relevant `TrueSightDAO` clones** in the workspace (`git fetch`, `git log` on default branch for the target day, **`gh pr list --state merged`** filtered by **`mergedAt`**) so digests reflect what actually merged, not memory. **§ Gathering Telegram Chat Logs** — same workbook’s **[Telegram Chat Logs](https://docs.google.com/spreadsheets/d/1qbZZhf-_7xzmDTriaJVWj6OZshyQsFkdsAV8-pyzASQ/edit#gid=0)** tab (~24h meaningful rows); helper **`market_research/scripts/list_recent_telegram_chat_logs_for_digest.py`**. **After every send** (Beer Hall or Founder Haus): **`market_research/scripts/append_openclaw_beer_hall_log.py`** must append a row to tab **`OpenClaw Beer Hall updates`** on the [Telegram compilation sheet](https://docs.google.com/spreadsheets/d/1qbZZhf-_7xzmDTriaJVWj6OZshyQsFkdsAV8-pyzASQ/edit) in the **same session** — OpenClaw alone does not update the sheet; see **`OPENCLAW_WHATSAPP.md` § Closed loop**. **If `openclaw message send` reports gateway timeout:** check gateway logs or the chat **before** resending — the message often **already arrived**; blind retries duplicate posts (**`OPENCLAW_WHATSAPP.md` § Gateway timeout vs send failure**). **GitHub in those two channels:** only **`https://github.com/TrueSightDAO/`** — do not link personal or other org repos (e.g. `garyjob`, `KrakeIO`) in Beer Hall / Founder Haus digests.
