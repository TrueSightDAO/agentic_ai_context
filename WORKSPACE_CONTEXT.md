...
- **Agroverse Shop CI/testing**: agroverse_shop has Playwright visual consistency tests. Run `npm test` in `agroverse_shop/`. See agroverse_shop `tests/README.md`. CI: `.github/workflows/visual-consistency.yml` on push/PR to main. Tests run against live site (beta or prod depending on repo). Smart runner: `npm run test:resume` to resume from failures.
