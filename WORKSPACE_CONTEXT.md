...
| **Infra & Tools** | jarvis, read_page, iching_oracle, point-of-sales-integrations, heierling-pos, **Cypher-Defense**, **truesight_autopilot** | Local LLM, browser extension, oracle app, POS integrations; **Cypher-Defense** — Web3 scam/phishing extension **and** AWS incident-response scripts + `docs/incidents/` (local `.env` for AWS keys; never commit); **truesight_autopilot** — autonomous SRE + developer that monitors email/AWS/GCP health, diagnoses failures with DeepSeek-V3, and opens fix PRs (never auto-merges) |
