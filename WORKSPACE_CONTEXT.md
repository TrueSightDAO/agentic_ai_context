...
| Group | Repos | Purpose |
|-------|--------|---------|
| **DAO / Agroverse** | dapp, truesight_me, tokenomics, agroverse_shop, agroverse-inventory (public JSON snapshot repo), qr_codes, proposals | DAO DApp, static site, tokenomics automation, e‑commerce, inventory snapshot host, QR codes, proposals |
...
- **Edgar** = **sentiment_importer** (edgar.truesight.me): DAO submission API; receives DApp contributions and triggers webhooks via Sidekiq (e.g. proposal → GitHub PR without waiting for cron). **Agroverse store inventory snapshot:** **`AgroverseInventorySnapshotPublishWorker`** **`GET`**s **`update_store_inventory`** (env **`AGROVERSE_INVENTORY_GAS_WEBAPP_URL`**, **`AGROVERSE_INVENTORY_PUBLISH_SECRET`**) after **Meta** Wix order **`MetaCheckoutOrderSync`** **`:created`** (Stripe webhook + success page), after **QR + Stripe** sale persistence in **`QrCodeCheckController`**, and—when **`enqueue_agroverse_inventory_snapshot`** is set—after a successful ledger **`WebhookTriggerWorker`** run (**`processTokenizedTransactions`**, **`processNonAgl4Transactions`**, inventory movement **`processTelegramChatLogs`**, expense **`parseAndProcessTelegramLogs`**). Level 2 sales **`parseTelegramChatLogs`** (Telegram → QR Code Sales only) does **not** chain snapshot. See **`API_CREDENTIALS_DOCUMENTATION.md`** §1.
...
- **agroverse_shop** ↔ **tokenomics (GAS `update_store_inventory`)** ↔ **`TrueSightDAO/agroverse-inventory`:** Live store inventory on the site is computed from DAO ledgers / **`Agroverse SKUs`** (see GAS **`update_store_inventory`**, clasp mirror **`tokenomics/clasp_mirrors/1P0Mg33i_dD9x9IeoHYvtKrf0xFcmUznpqAswyC_KXR3VJZu-0C-UOP0v/`**, deployment URL in **`Code.js`**). A **public JSON snapshot** (intended read path: **`raw.githubusercontent.com`**) may be written to the dedicated **`agroverse-inventory`** repo via the GitHub Contents API. The fine-grained PAT for that write lives only in **Apps Script → Project Settings → Script properties** as **`AGROVERSE_INVENTORY_GIT_REPO_UPDATE_PAT`** (never commit; not a **`sentiment_importer`** `.env` variable unless server-side code also pushes to GitHub). See **`API_CREDENTIALS_DOCUMENTATION.md`** and **`PROJECT_INDEX.md`** (**agroverse-inventory** row).
