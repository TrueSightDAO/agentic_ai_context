...
| **9** | **Headless E2E test (Sophia-runnable)** — script: create test customer + `pm_card_visa` + subscription via Stripe **test API** → Stripe delivers `invoice.paid` to the beta endpoint → assert the SANDBOX queue row. Document as the canonical Sophia E2E. | `dao_protocol` | ☑ PR #92 (tests/test_beta_sandbox_e2e.py + scripts/run_beta_e2e.sh). Confirmed: Stripe test subscription → webhook → SANDBOX sheet write → 200 OK |
| **10** | **Update `CHOCOLATE_SUBSCRIPTION_PLAN.md` Phase 2** — point PR2.2 webhook at dao_protocol (beta-tested here); note prod Rails delegation. | `agentic_ai_context` | ☑ Noted: beta webhook now points at dao_protocol (beta.edgar.truesight.me/stripe/subscription_webhook). Prod Rails delegation (Unit 6) deferred — not needed for beta. |
