...
|------|---------|-----------|--------|-------|
| 2026-08-22 | 2026TEST_20260822_1 (AGL4) | N/A — status set directly, no sales event | ✅ PASS | Full E2E verified: QR → ASSIGNED_TO_TREE + tree fields; SunMint TEST-20260822-1 → LINKED; main ledger offchain transactions rows 4127/4128 = fulfillment pair ("SunMint Tree Planting Contract - agl4", -1 "Cacao Tree To Be Planted" / +1 "Cacao Tree Planted", TRUE). QR → INVALIDATED per §10 step 6. Note: first attempt via deployment @15 wrote to AGL4 sub-ledger (old shape) — root cause: anonymous deployment pinned to pre-#406 code; redeployed @16 (current HEAD) → correct main-ledger booking. First run attempted a real $0 sales-event submission per the original design; superseded same-day after Gary flagged that real sales-pipeline submissions must never be used for tests, even at $0. That attempt also surfaced a real, separate production bug (tokenomics #407, $0-price sales silently ignored) — worth fixing independently, no longer a dependency of this procedure. |
| 2026-08-22 | TEST_AGL4_20260822_2 (AGL4) | N/A — status set directly, no sales event | ✅ PASS | Full E2E verified on the REAL flow (governor-requested variant: Owner Email = garyjob@gmail.com instead of placeholder): QR TEST_AGL4_20260822_2 → SOLD (sheet edit) → ASSIGNED_TO_TREE + tree fields; SunMint TEST-20260822-2 → NEW → LINKED (QR + timestamp written); Tree Planting Link tracking row 15: LINKED/OK (Edgar_20260822234048_073, 23:41:00Z, Gary Teh); main ledger offchain transactions rows 4133/4134 = fulfillment pair ("SunMint Tree Planting Contract - agl4", -1 "Cacao Tree To Be Planted" / +1 "Cacao Tree Planted", TRUE); BOTH emails genuinely delivered to garyjob@gmail.com — tree-planted notification 23:41:00Z (LINK-flow send, first genuine one — the deployment OAuth fix works) AND onboarding email 23:42:51Z (processBatch) — verified in admin@truesight.me Sent. QR → INVALIDATED per §10 step 6. Note: first LINK attempt 500'd with transient SSLEOFError (dropped outbound TLS during Sheets write); plain retry → HTTP 200, signature verified (matches governor's diagnosis). |
| 2026-08-22 | TEST_AGL4_20260822_3 (AGL4) | N/A — status set directly, no sales event | ✅ PASS | Repeatability run #2 on the REAL flow: QR TEST_AGL4_20260822_3 → SOLD (sheet edit) → ASSIGNED_TO_TREE + tree fields; SunMint TEST-20260822-3 → NEW → LINKED (QR + timestamp 23:49:07Z); Tree Planting Link tracking row 11714: LINKED/OK (Edgar_20260822234856_075, 23:49:08Z); main ledger offchain transactions rows 4137/4138 = fulfillment pair ("SunMint Tree Planting Contract - agl4", -1 "Cacao Tree To Be Planted" / +1 "Cacao Tree Planted", TRUE); BOTH emails genuinely delivered to garyjob@gmail.com — tree-planted notification 23:49:08Z (LINK-flow autosend, no human intervention — OAuth fix holds) AND onboarding email 00:42:51Z (processBatch via the #417 self-installed hourly trigger — fired exactly on schedule the next hour, proving the trigger is live and reliable, NOT a manual run). Manifest pipeline validated: seed_from_sheet.py generated qrs/TEST_AGL4_20260822_3.json (ASSIGNED_TO_TREE, 44.6/-123.3); https://truesight.me/qr/?id=TEST_AGL4_20260822_3 renders full page (§11 Node-VM: QR img, coords, photo, OSM map, no errors). QR → INVALIDATED per §10 step 6. First LINK attempt succeeded (no SSLEOFError this run). | (governor-requested variant: Owner Email = garyjob@gmail.com instead of placeholder): QR TEST_AGL4_20260822_2 → SOLD (sheet edit) → ASSIGNED_TO_TREE + tree fields; SunMint TEST-20260822-2 → NEW → LINKED (QR + timestamp written); Tree Planting Link tracking row 15: LINKED/OK (Edgar_20260822234048_073, 23:41:00Z, Gary Teh); main ledger offchain transactions rows 4133/4134 = fulfillment pair ("SunMint Tree Planting Contract - agl4", -1 "Cacao Tree To Be Planted" / +1 "Cacao Tree Planted", TRUE); BOTH emails genuinely delivered to garyjob@gmail.com — tree-planted notification 23:41:00Z (LINK-flow send, first genuine one — the deployment OAuth fix works) AND onboarding email 23:42:51Z (processBatch) — verified in admin@truesight.me Sent. QR → INVALIDATED per §10 step 6. Note: first LINK attempt 500'd with transient SSLEOFError (dropped outbound TLS during Sheets write); plain retry → HTTP 200, signature verified (matches governor's diagnosis). |
