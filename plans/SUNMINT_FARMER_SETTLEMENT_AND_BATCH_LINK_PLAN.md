...
> **RESUME HERE → PR5** — **PR4 complete 2026-09-20** (`tokenomics` #533/#534/#535 + `dao_protocol` #174; GAS source-only, no deploy). A tree-id-bearing `[PAYOUT EVENT]` now books the SunMint settlement beside the existing Tier-1 tracking write: step 1 confirmed a new tag was unnecessary — `[PAYOUT EVENT]` was **already routed** (#170) **and already catalogued with `Tree Planting IDs`** (§1.7/§1.10 corrected below); committed (`Linked QR Code` populated) ⇒ cross-ledger 3-leg transfer (§0.11), uncommitted ⇒ main-ledger 3-leg pool settlement (§1.2 Path B); any resolution failure **fails closed** (`LEDGER_NOT_BOOKED`, nothing written). Next: **PR5** — revise `process_tree_planting_link.gs` per §1.3/§1.4 (independent `-1 Cacao Tree To Be Planted` always fires; source-dependent pool-vs-unpaid-liability effect; reimbursement transfer only in the cross-ledger pool case). **First real use — Paulo’s 10-tree backfill (§1.1) — remains under the ledger-money gate (§2).**
>
