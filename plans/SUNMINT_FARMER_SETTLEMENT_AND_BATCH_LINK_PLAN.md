...
| 0.8 (RESOLVED 2026-09-20) | **Aging policy for stalled prepayments or unpaid confirmed trees** | Ruled by Gary: **passive report** (not active nudging). Built as **PR8** — a read-only, no-outreach aging report (`truesight_autopilot` #497, squash `47e2d920`). Non-blocking. |
...
| 0.13 (**REVERSED** 2026-09-20 — was RULED then refuted; see §8.5) | **The `Cacao Tree To Be Planted` liability currency literal is CONSTANT (mixed case) on every ledger — NOT per-ledger** | **Refuted against RAW sheets 2026-09-20:** `Transactions` col E of **SEF1**, **PP1**, **AGL6** and main's `offchain transactions` ALL read `Cacao Tree To Be Planted` *(mixed case)* — byte-identical. The earlier 'managed ledgers use UPPERCASE' evidence came from **`snapshot_managed_ledgers.py` L91 (`… .upper()`)** — a snapshot-tool artifact that uppercases *every* currency key (it also turns `Kraft Pouches` → `KRAFT POUCHES`). Corroborated independently by Envoy. The managed-ledger contributor is likewise suffixed per-ledger (`SunMint Tree Planting Contract - <agl>`, `RESERVATION_EVENT_SPEC.md` leg 3). **No defect: PR5's hard-coded mixed-case `TPL_CUSTOMER_LIABILITY_LITERAL` (`process_tree_planting_link.js` L81) matches every ledger's actual spelling. Unit PR5.2 WITHDRAWN; Decision 0.13 REVERSED (§8.5).** |
| 0.14 (RULED 2026-09-20) | **Financing is PER PLOT, and a `[PLOT FINANCING EVENT]` declares the tree count N** | Gary: financing is **per plot** — a farmer may hold **many** plots, so an advance is scoped to (plot, N trees), not to the farmer. The new **`[PLOT FINANCING EVENT]`** is how the DAO advances cash up front to finance **N** trees on one plot; it is the **opposite direction** to `[FARMER PAYMENT EVENT]` (an advance leaves main *before* any sale exists; a payout settles a liability *after* one). |
| 0.15 (RULED 2026-09-20) | **A `[PLOT FINANCING EVENT]` mints N pool units on main and seeds the plot registry (col T)** | The advance books exactly **two legs on the MAIN ledger** — cash OUT `-amount` (Is Revenue **blank**, matching PR4's cash-out convention) and a pool mint `+N \`Cacao Tree Planted - Unassigned\`` (Is Revenue `N`, matching PR3's settlement rows) — and **writes the `SunMint Plots` col T `Contributor Name`** with the farmer. That col-T write is what makes a later **plot-level link bookable**; while col T is blank a plot link **fails CLOSED**. **FAIL-CLOSED**: an unresolved plot row, or a non-positive amount / tree count, books **nothing** and flags the tracking row. Shipped PR10b (`tokenomics` #538). |
| 0.16 (RULED 2026-09-20 — Q7v5) | **The per-tree infrastructure charge is a DEDICATED `Currencies` col U `Tree Charge`, keyed by the tree literal** | The amount a `[TREE PLANTING LINK EVENT]` transfers off a managed ledger at link time is the **infra charge** — **`Currencies` col U `Tree Charge`** (keyed by the tree literals `Cacao Tree To Be Planted` / `Cacao Tree Planted`), resolved with per-ledger-agnostic normalisation and **failing CLOSED** if absent/unparseable. It is **distinct** from both the retail/AUM price (`Currencies` col **B**) and the farm cost (`SunMint Tree Planting` col **P `Cost of Tree`**), and from the **financing advance** (0.15). Cash moves **QR-ledger → main at link** for **both** tree-level and plot-level links (Q1); the cash-leg contributor is the **linking operator** (Q2); an unknown / blank / invalidated target is a **hard reject surfaced at the interface** (Q3); a **PROPOSED** (non-invalidated) plot is linkable (Q6). Shipped PR5.3a/b + PR6.2. |
...
| **PR5.3a/b** | Link-time cash transfer. The amount is the per-tree **INFRA charge** = `Currencies` col U `Tree Charge` on the **main** ledger, keyed by the tree literal, case-insensitive — deliberately NOT col B (retail/AUM) and NOT SunMint col P (farm cost). New `tplNormalizeAmount_` parses currency-suffixed cells (fixes PR5's fail-OPEN `Number("1.5 BRL")===NaN` silent skip); an unbookable charge **fails CLOSED** (caller rejects before any write). Fires in **BOTH** pool+committed when QR≠main (Q1); `null` = no transfer intended (plot path). | `tokenomics` |
| ~~**PR5.2** (Decision 0.13)~~ **WITHDRAWN 2026-09-20** | ~~Resolve the tree-liability currency literal per ledger.~~ **No defect exists — Decision 0.13 refuted (§8.5): every ledger books `Cacao Tree To Be Planted` (mixed case), so PR5's hard-coded literal is already correct. Nothing to build.** A defensive case-insensitive comparator in the link handler is optional future-proofing only, not 0.13. | `tokenomics` |
...
> **RESUME HERE → 🅿️ PARKED (2026-09-20) — the build is CODE + DOCS COMPLETE; no ungated unit remains.** The `[PLOT FINANCING EVENT]` vertical (surfaced as **GAP A** at Envoy's 2026-09-20 "build complete" review) is now fully documented: `tokenomics` **#543** (`26a96d5`; `SCHEMA.md` **`Plot Financing`** sheet + pool mint/consume note, `API.md` **§12**) and **this PR** adds **§0 Decisions 0.14–0.16** + the **PR10c** tracker row. **Decision 0.8 is now RESOLVED** (Gary ruled: **passive report**, not active nudging) and **PR8 SHIPPED** — `truesight_autopilot` **#497** (`47e2d920`), a read-only no-outreach aging report (`scripts/sunmint_stalled_balance_aging_report.py`, 9/9 tests). The **only** remaining item is the **RUN** (**§2**; held personally by Gary). **Read-only/docs-only — no code, no `clasp`, no ledger write.**
>
> **Prior RESUME head was PR9 (docs)** — SHIPPED 2026-09-20 (`tokenomics` #542, `3c86bf2d`); this plan's marker/tracker close-out landed then. PR9 finalizes the reference docs for the whole plan: `tokenomics/SCHEMA.md` gains the **`Currencies` col U `Tree Charge`** row (per-tree infra charge; NOT col B retail/AUM, NOT `SunMint Tree Planting` col P farm cost) and the **`SunMint Plots` col T `Contributor Name`** row (+ the invalidated-plot eligibility note), and the stale blanket note *"tree-planting literals are never a `Currencies` tab row"* is **corrected** — the three line-item literals are plain ledger strings, but the two tree-charge literals (`Cacao Tree To Be Planted` / `Cacao Tree Planted`) legitimately hold `Currencies` rows carrying the charge (Q7v5). `tokenomics/API.md` §9 `[TREE PLANTING LINK EVENT]` gains the **`Plot ID`** variant (PR6). The plan's own PR9 line also listed a `GAS_SCRIPT_PROPERTIES.md` routing-table correction — that item is a **NO-OP**: PR2 proved live `dispatch.py` already routes `[TREE PLANTING EVENT]`/`[TREE PLANTING REJECT EVENT]`/`[PAYOUT EVENT]`/`[TREE PLANTING LINK EVENT]`/`[ASSET RECEIPT EVENT]`, so the earlier "none of them" wording in this plan was itself the error (already corrected). **Docs-only — no code, no `clasp`, no ledger write.** **Next after PR9:** only the **optional PR8** (aging report, pending Decision 0.8, non-blocking) and the **gated RUN** (§2; HELD by Gary personally).
>
> Prior RESUME head was **PR6.2** (plot-level link path); shipped + ticked 2026-09-20. Prior unit: **PR5.3a/b complete 2026-09-20** (`tokenomics` #539/#540, `77db46a7`). PR5.3a/b replaced PR5's fail-OPEN transfer: the link-time cash amount now comes from a DEDICATED `Currencies` col U `Tree Charge` keyed by the tree literal (NOT col B retail/AUM, NOT SunMint col P farm cost), through a new `tplNormalizeAmount_` (so `"1.5 BRL"` → 1.5 — the merged-PR5 `Number("1.5 BRL")===NaN` silent-skip is gone), and an unbookable charge **fails CLOSED** (leg list `[]`; the caller rejects the row up front, before any QR/SunMint write). Fires in **BOTH** pool+committed when QR≠main (Q1); `null` = no transfer intended (the plot path, until PR6.2). Verified: harness **35/35**, guard **18/18**, `node --check` OK, `pytest` baseline unchanged, no duplicate top-level consts. **No ledger event, no `clasp` deploy.** Prior-prior: **PR7b complete 2026-09-20** (`dao_protocol` #176, `c537ce1f`). PR7b collapsed the PR7 over-count: eligible `SunMint Tree Planting` rows are grouped to **one unit per physical tree** by **photo-URL identity** (138 rows → 125 trees; 50 duplicate rows collapsed), because Gary's *option (b)* row-level key (col A `Telegram Update ID`) duplicates **identically to col D** and would not have fixed it — the twins are byte-identical re-ingests of the same tree, and the handler links only the first `NEW` row. A tree group is skipped if **any** of its rows is already linked, so a linked tree is never re-offered. Live re-run: **9/9 pairings → distinct trees**. Dry-run default + `--execute` needs `--yes` unchanged. Verified: tests **18/18**, full pytest **206 passed** (pre-existing fails/collection-errors unchanged, re-confirmed on `main`). **No ledger event, no `clasp` deploy.** **⚠️ Decision 0.12 RETRACTED 2026-09-20:** Gary first ruled the purchase asset assignable to any ledger, then retracted (*"We should only have it on the main"*). Purchases stay **main-only**; units **PR2b/PR3.2 are WITHDRAWN** (never built); **no merged code needs reverting** (PR3–PR7b as-built assume main-only ⇒ correct). §8 kept as a record; the only survivor is **PR5.2** (Decision 0.13). **Decision 0.13 (§8.5): ⚠️ REVERSED 2026-09-20** — the tree-liability currency literal is **constant** (`Cacao Tree To Be Planted`, mixed case) on **every** ledger; the 'per-ledger/UPPERCASE' claim was a **snapshot-tool artifact** (`snapshot_managed_ledgers.py` L91 `.upper()`). PR5's hard-coded literal is **correct**; **PR5.2 is WITHDRAWN, nothing to build.** **✅ PR6.2 shipped 2026-09-20 (`tokenomics` #541, `bf28f934`):** plot-level links now book the **same** `Currencies` col U `Tree Charge` as tree-level links (resolved UP FRONT, fails CLOSED) instead of the PR5.3b `null` sentinel, and `tplResolvePlotContributor_` now **hard-rejects an invalidated plot** (`SunMint Plots` `Status` col E `= invalid`, set by `process_plot_invalidation.gs`) so a retracted plot can never book a ledger effect — Q6 eligibility + Q3 bad-target rejection. New `TPL_PLOTS_STATUS_COL = 4` + pure `tplIsPlotStatusInvalid_`. GAS source-only. harness 38/38 + guard 20/20. **Active next unit: PR9 (docs).** **RUN (held, see below)** — Paulo's 10-tree backfill (§1.1) + first real batch link — under the **ledger-money gate (§2): explicit governor go only**, and **Gary has personally held it** (*“Don’t do the live run… let me look at it and decide”*). Open prerequisite before RUN: the **`SunMint Plots` `Contributor Name` (col T) backfill**, now reframed by Gary as **proximity-computed** (“the plot is linked to the farm by proximity”) rather than manually typed — scoped as a **separate follow-up** (🚩 open questions filed in `OPEN_FOLLOWUPS.md`), not built; until done, plot links fail closed.
...
>
> **Open before certain units:** PR2/PR4's `clasp pull` verifications are **done** — §0.9's Currency allowlist is open-ended (PR2) and §0.10's payout handler is log-only (PR4 builds the booking). §0.11 is ruled. Remaining open: Decision 0.7 (reconciliation-match automation — default stated); Decision 0.8 (aging policy) is now **RESOLVED** (passive report, PR8). Non-blocking.

> **PR10c shipped 2026-09-20 — GAP A closure (docs-only, no code, no `clasp`, no ledger write):** the `[PLOT FINANCING EVENT]` vertical (PR10a `dao_protocol` #177 `c68718a`, PR10b `tokenomics` #538 `1beabd0`) had shipped **code with zero documentation footprint**. Closed in two halves — **`tokenomics` #543** (`26a96d5`) adds the `Plot Financing` sheet section + the pool mint/consume note (`SCHEMA.md`) and the event section (`API.md` §12); **this PR** adds **§0 Decisions 0.14–0.16** (per‑plot financing; the N‑unit mint + col‑T seeding; the col‑U infra-charge rule) and the **PR10c** row below. Named **PR10c** (NOT the plan's PR8) to avoid clobbering the plan's own **PR8 = aging report**. The event is already registered in the Edgar catalog (v10, `PLOT FINANCING EVENT`) and routed via `dispatch.py`.

> **PR8 shipped 2026-09-20 — Decision 0.8 RESOLVED (passive report).** Gary ruled the aging policy as a **passive report**, *not* active nudging. Built in **`truesight_autopilot`** (the plan allowed `dao_client` *or* `truesight_autopilot`): **#497** (squash `47e2d920`) adds `scripts/sunmint_stalled_balance_aging_report.py` — a **read-only** aging report over the main ledger's `offchain transactions` tab that tracks the two stalled literals (`Cacao Tree Purchased - Not Planted`, `Cacao Tree - To Be Paid For`), computes outstanding balances + 30/60/90-day aging buckets against a configurable `--threshold-days` (default 30), and emits text or `--json`. **Case-insensitive** literal matching (PR5.2 comparator precedent) + tolerant amount parsing (`R$`/`$`/commas/parens; `N/A` → skip) — fail-soft, never books. **No outreach, no ledger write, no `clasp`.** Tests `tests/test_sunmint_stalled_balance_aging_report.py` **9/9**; full suite green (5 unrelated pre-existing failures in `test_resolve_tdg_issued.py` exist on clean `main`). Contribution = explicit-go gate.
...
| PR5.3a/b (link-time tree transfer via `Currencies` col U `Tree Charge`; normalize + fail CLOSED) | ☑ | ☑ | ☐ | <!-- Merged tokenomics#539 (77db46a7) + #540; replaces PR5's fail-OPEN transfer (Number("1.5 BRL")===NaN); col U not col B/P; fires BOTH pool+committed when QR≠main; null=no-transfer (plot path). harness 35/35 + guard 18/18. Contribution = explicit-go gate -->
| PR6.2 (plot link books the col-U transfer; invalidated-plot eligibility filter) | ☑ | ☑ | ☐ | <!-- Merged tokenomics#541 (bf28f934); plot branch now resolves the SAME Currencies col U Tree Charge (up front, fails CLOSED) instead of null; tplResolvePlotContributor_ hard-rejects Status='invalid' plots (col E; process_plot_invalidation.gs), Q6 + Q3. New TPL_PLOTS_STATUS_COL=4 + pure tplIsPlotStatusInvalid_. harness 38/38 + guard 20/20. Contribution = explicit-go gate -->
...
| ~~PR5.2~~ (per-ledger liability currency literal — Decision 0.13 **REVERSED 2026-09-20, WITHDRAWN**; no code) | — | — | — |
| PR8 (aging report — Decision 0.8 = passive report; read-only) | ☑ | ☑ | ☐ | <!-- Merged truesight_autopilot#497 (47e2d920): read-only stalled-balance aging report for 'Cacao Tree Purchased - Not Planted' + 'Cacao Tree - To Be Paid For'; passive (no outreach), no ledger write; 9/9 tests, full suite green (5 unrelated pre-existing failures on clean main). Contribution = explicit-go gate -->
| PR9 (docs, incl. the GAS_SCRIPT_PROPERTIES.md routing-table correction) | ☑ | ☑ | ☐ | <!-- Merged tokenomics#542 (3c86bf2d): SCHEMA.md Currencies col U Tree Charge + SunMint Plots col T Contributor Name + corrected the stale "never a Currencies tab row" literal note; API.md §9 Plot ID variant. GAS_SCRIPT_PROPERTIES routing item is a NO-OP (PR2). Docs-only, no clasp, no ledger write. Contribution = explicit-go gate -->
| PR10c (docs — `[PLOT FINANCING EVENT]` plan §0.14–0.16 + this row; GAP A closure) | ☑ | ☑ | ☐ | <!-- Docs-only; tokenomics#543 (26a96d5) SCHEMA.md `Plot Financing` sheet + API.md §12; plan §0 Decisions 0.14-0.16 + this row. Contribution = explicit-go gate -->
...
- Decision 0.7 (automatic vs. governor-confirmed reconciliation match) — still flagged; does not block PR1.
- Decision 0.8 (aging policy) — **RESOLVED 2026-09-20** (passive report; shipped as PR8).
...
- §0.9's main-only caveat **REINSTATED 2026-09-20 (Decision 0.12 retracted)** — Gary first ruled the purchase asset
  ...
  **withdrawn**. PR3–PR7b as-built assume main-only ⇒ **correct, nothing to revert.** §8 retained as a
  record; **PR5.2** (Decision 0.13) was the only survivor, and it too is now **WITHDRAWN** (0.13 reversed — see §8.5).
...
3. ~~**PR5.2 (if needed):** make `tplResolveSource_` scan the funding ledger(s).~~ **Withdrawn** with the 0.13 reversal (§8.5).
...
- **Q7 — Per-ledger currency literals (Decision 0.13, §8.5):** ~~confirm the direction~~ **MOOT — 0.13 REVERSED 2026-09-20 (§8.5).** There is no per-ledger spelling to resolve: every ledger books the mixed-case literal, so neither (a) nor (b) is needed. **PR5.2 withdrawn.**
