...
> **RESUME HERE → PR7** — **PR6 complete 2026-09-20** (`tokenomics` #537 + `dapp_beta` #103; GAS source-only, no deploy). `process_tree_planting_link.gs` now also accepts a **Plot ID** as an alternative to a SunMint submission row (§1.6): a plot-fallback branch resolves the farmer from the **`SunMint Plots` registry** — the new **col T `Contributor Name`** (Envoy, 2026-09-20: on money-adjacent logic the registry holds the fact, never the submission payload; a wrong fuzzy/payload attribution could discharge the wrong farmer's balance) — writes QR **`Linked Plot ID`** (col AC, PR1) + a representative plot image from `sunmint/plots/media.json` (first `kind:image`, else the first video's `thumbnail`), and reuses PR5's `appendTreePlantingLedgerFulfillment_` sourced from the plot's farmer. A plot has **no per-tree `Cost of Tree`**, so the pool-source reimbursement-transfer amount is unbookable ⇒ plot links book **no cash legs** (fail closed; only the money leg is UAT-gated). Consumption accounting: one plot link = one QR = **one pool unit** (1:1, same as a tree-level link). Fails closed when the plot is unknown or its `Contributor Name` is blank. Next: **PR7** — batch allocation tool (`dao_client`; distinct-email → tree, repeat/no-email → plot; dry-run first). **First real use — Paulo's 10-tree backfill (§1.1) — remains under the ledger-money gate (§2).**
>
