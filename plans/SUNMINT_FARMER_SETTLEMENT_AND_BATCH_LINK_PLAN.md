...
> **RESUME HERE → PR6** — **PR5 complete 2026-09-20** (`tokenomics` #536; GAS source-only, no deploy). `process_tree_planting_link.gs` now books the link-time effect per §1.3/§1.4: the old fixed `-1 To Be Planted` / `+1 Cacao Tree Planted` pair is gone; instead an **independent `-1 Cacao Tree To Be Planted`** always fires (discharging the sale-time customer liability, §1.3) plus a **source-dependent** effect — **pool source** also books `-1 Cacao Tree Planted - Unassigned` (on main) **and, only when the QR's ledger ≠ main, the reimbursement transfer** (`-cash` QR ledger / `+cash` main); **committed source** books the discharge leg alone (the `To Be Paid For` liability stays open). The source is **derived from the farmer's aggregate pool balance** (`Cacao Tree Planted - Unassigned` balance ≥ 1 ⇒ pool), mirroring PR3's farmer-aggregate abstraction — the ledger derives the fact, the submitter never asserts it (Envoy, 2026-09-20). Fails closed; partial writes flagged not rolled back. Next: **PR6** — plot-level link path: accept a plot id (`sunmint/plots/index.geojson`) as an alternative to a SunMint submission row, write **`Linked Plot ID`** + a representative plot image, **with its own consumption accounting** (a plot spans many trees' worth of pool supply — small design pass at PR6 start). **First real use — Paulo’s 10-tree backfill (§1.1) — remains under the ledger-money gate (§2).**
>
