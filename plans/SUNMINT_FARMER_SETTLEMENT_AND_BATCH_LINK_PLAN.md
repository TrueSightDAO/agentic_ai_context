...
| 0.9 | **`[TREE PURCHASE EVENT]` reuses the existing `[ASSET RECEIPT EVENT]` mechanism** | Verified live 2026-09-20 (§1.10): the `Asset Receipts` tab + `asset_receipt_ingest` GAS project + `dao_client`'s existing `report_asset_receipt.py` CLI already do exactly this shape (pay cash, book an offchain asset row). No new event, no new CLI needed for v1 — submit with `Currency = "Cacao Tree Purchased - Not Planted"`. **RESOLVED 2026-09-20 (Gary ruling — Decision 0.12): the main-only caveat is superseded.** The purchase asset `Cacao Tree Purchased - Not Planted` **must be bookable on any managed ledger**, because the source of funds can come from any ledger (including main). Implementation = give the `[ASSET RECEIPT EVENT]` ingest a **ledger-target field**, mirroring `[DAO Inventory Expense Event]`'s existing `Target Ledger` label (validated in `dao_protocol` against the treasury cache's `ledgers[]`, whose entries carry `ledger_name` + `ledger_url`), so the offchain leg lands on the designated ledger instead of the hard-coded `ASSET_RECEIPT_MAIN_SPREADSHEET_ID`. The CLI's existing `Destination Contribution File Location` label is an **attachment**-pairing field (§1.10), **not** a ledger selector — a distinct label is needed. **Ripple (already-merged PR3):** `reconcileTreePlanting_` reads the farmer's open balance from `contributorsSheetId` (main) only and books Path B on main always — with a managed-ledger purchase it would **miss the open balance and mis-book Path B**, double-counting. PR3 must be extended to locate the funding ledger. Until PR2b ships, purchases still default to main. |
...
| 0.12 (RULED 2026-09-20) | **`Cacao Tree Purchased - Not Planted` must be assignable to ANY managed ledger (not just main)** | Ruled by Gary: *"Cacao tree purchased - not yet planted needs to be assignable to any ledger. Since the source of funds can come from any ledger including our main one."* **The REQUIREMENT is ruled; the DESIGN is not yet signed off.** This promotes §0.9's phase-2 caveat to a **v1 architecture change**: the purchase asset's offchain leg lands on the **designated funding ledger** (a managed AGL, or main when the general treasury funds it). Because PR3–PR7b were all built against the v1 assumption *"all purchases fund main ⇒ pool units live on main,"* this is a **money-path change, not a tweak** — see **§8 (impact assessment)** for the affected units, the options, and the open questions. **No code until the redesign is signed off (Envoy, 2026-09-20).** |
| 0.13 (RULED 2026-09-20) | **The `Cacao Tree To Be Planted` liability currency literal is PER-LEDGER, not constant** | Flagged by Gary; **VERIFIED TRUE against live data 2026-09-20** (treasury-cache `managed-ledgers/*.json` summaries + the main ledger's `offchain transactions` Currency column): the **main** ledger books the sale-time liability as `Cacao Tree To Be Planted` *(mixed case)*, while **managed ledgers** book it as `CACAO TREE TO BE PLANTED` *(UPPERCASE)* — live on **SEF1** (4 rows), **PP1** (2 rows), **AGL6** (44 rows). The managed-ledger contributor is likewise suffixed per-ledger (`SunMint Tree Planting Contract - <agl>`, `RESERVATION_EVENT_SPEC.md` leg 3). **This is a silent defect in already-merged PR5**, which hard-codes the mixed-case main literal unconditionally (`process_tree_planting_link.js` `TPL_CUSTOMER_LIABILITY_LITERAL`, L81) — see **§8.5**. **Requirement RULED; fix design folded into the §8 redesign (unit PR5.2).** No code until sign-off. |
...
|---|---|---|
| `[TREE PURCHASE EVENT]` (new) | Governor | `-cash` + `+N Cacao Tree Purchased - Not Planted` **on the designated funding ledger** — any managed ledger, or main when the general treasury funds it (Decision 0.12) |
...
| **PR2b** (DEFERRED — Decision 0.12; **design pending sign-off, then gated GAS deploy**) | **Scope below is a *proposal*, not an approved design** (see §8). Extend the `[ASSET RECEIPT EVENT]` ingest (`asset_receipt_ingest/Code.gs`) with a **ledger-target field** so `Cacao Tree Purchased - Not Planted` books its offchain leg on **any** designated managed ledger (AGL n) or main, not the hard-coded main SS. Mirror `[DAO Inventory Expense Event]`'s `Target Ledger` label: add the label to `report_asset_receipt.py`'s `canonical_labels`, resolve the name → spreadsheetId via the treasury cache's `ledgers[]` (`ledger_name`+→`ledger_url`), validate server-side in `dao_protocol` (`_get_valid_expense_ledgers` precedent), and default to main when the field is absent (**fully backward-compatible**). `asset_receipt_ingest/Code.gs` is an **owned** project (env `ASSET_RECEIPT_MAIN_SPREADSHEET_ID`), so the GAS source edit ships by PR; the `clasp push` is the always-stop gate. | `tokenomics` + `dao_protocol` |
| **PR3** | Extend `process_tree_planting_telegram_logs.gs` (SunMint Tree Planting project, §1.10): on each new confirmed submission, query the `Cacao Tree Purchased - Not Planted` ledger balance for that farmer (`Contributor Name`, §1.7 — FIFO). Match found → `-1 Purchased-Not-Planted / +1 Planted-Unassigned`, write `Payment Event Ref`, **emit the system-signed reconciliation event**. No match → `+1 Cacao Tree - To Be Paid For` on main ledger. **⚠️ Decision 0.12 ripple (PR3.2):** `reconcileTreePlanting_` currently reads the open balance from **main** (`contributorsSheetId`) only — with a managed-ledger purchase it misses the balance and mis-books Path B. Must resolve the funding ledger (§8). | `tokenomics` |
...
| **PR5.2** (DEFERRED — Decision 0.13; **design pending sign-off**, then gated GAS deploy) | Resolve the tree-liability **currency literal per ledger** instead of hard-coding main's. Live data shows main uses `Cacao Tree To Be Planted` (mixed case) but managed ledgers use `CACAO TREE TO BE PLANTED` (uppercase). PR5's `TPL_CUSTOMER_LIABILITY_LITERAL` must become a **per-ledger lookup** (main/+AGL4 → mixed case; managed → uppercase), and the `Planted - Unassigned` / `To Be Paid For` literals checked the same way. Without this, a managed-ledger QR link writes a **spurious second liability** (the ingest auto-creates a `Currencies` row for any new literal — PR2) instead of discharging the real one. | `tokenomics` |
...
> **RESUME HERE → RUN (ledger-money gate, HELD by Gary) — **PR7b complete 2026-09-20** (`dao_protocol` #176, `c537ce1f`). PR7b collapsed the PR7 over-count: eligible `SunMint Tree Planting` rows are grouped to **one unit per physical tree** by **photo-URL identity** (138 rows → 125 trees; 50 duplicate rows collapsed), because Gary's *option (b)* row-level key (col A `Telegram Update ID`) duplicates **identically to col D** and would not have fixed it — the twins are byte-identical re-ingests of the same tree, and the handler links only the first `NEW` row. A tree group is skipped if **any** of its rows is already linked, so a linked tree is never re-offered. Live re-run: **9/9 pairings → distinct trees**. Dry-run default + `--execute` needs `--yes` unchanged. Verified: tests **18/18**, full pytest **206 passed** (pre-existing fails/collection-errors unchanged, re-confirmed on `main`). **No ledger event, no `clasp` deploy.** **⚠️ NEW (Decision 0.12, 2026-09-20):** Gary ruled the purchase asset must be **assignable to any ledger** (source of funds can come from any, incl. main). Impact assessment in **§8** — this is a **money-path redesign**, not a tweak: PR3/PR3's pool concept/PR5/PR7/PR4 all assumed *"purchases fund main ⇒ pool units live on main."* **Design NOT signed off; no code until the governor answers Q1–Q6 (§8.4) and approves the redesign.** **First next action = get that sign-off** (a governor decision, not a PR). **Decision 0.13 (§8.5):** the tree-liability currency literal is **per-ledger** (main=`Cacao Tree To Be Planted`; managed=`CACAO TREE TO BE PLANTED`) — verified live; PR5's hard-coded literal is a **silent money-path defect** (unit PR5.2). Next: **RUN** — Paulo's 10-tree backfill (§1.1) + first real batch link — under the **ledger-money gate (§2): explicit governor go only**, and **Gary has personally held it** (*“Don’t do the live run… let me look at it and decide”*). Open prerequisite before RUN: the **`SunMint Plots` `Contributor Name` (col T) backfill**, now reframed by Gary as **proximity-computed** (“the plot is linked to the farm by proximity”) rather than manually typed — scoped as a **separate follow-up** (🚩 open questions filed in `OPEN_FOLLOWUPS.md`), not built; until done, plot links fail closed.
...
| PR2b (any-ledger `Purchased - Not Planted` — Decision 0.12; **⚠️ design pending sign-off**, §8) | ☐ | ☐ | ☐ |
| PR3.2 (PR3 reads funding ledger, not main-only — part of the §8 redesign) | ☐ | ☐ | ☐ |
| PR5.2 (per-ledger liability currency literal — Decision 0.13; **⚠️ design pending sign-off**, §8.5) | ☐ | ☐ | ☐ |
...
- §0.9's caveat **RESOLVED 2026-09-20 (Decision 0.12)** — Gary ruled the purchase asset must be
  assignable to **any** ledger (source of funds can come from any, incl. main). Promoted from phase-2
  to v1: **PR2b** extends the `[ASSET RECEIPT EVENT]` ingest with a `Target Ledger` field (mirroring
  `[DAO Inventory Expense Event]`), and **PR3.2** fixes the reconciliation to read the funding
  ledger instead of main-only. Until PR2b ships, purchases still default to main.
...
## 8. Impact assessment — Decision 0.12 (purchases fundable from ANY ledger)

**Status: requirement RULED by Gary; design NOT yet signed off. No code until the governor
approves the redesign (Envoy, 2026-09-20). RUN stays held regardless.**

### 8.1 The v1 assumption being changed

§0.9's original v1 simplification was *"all purchases fund main ⇒ pool units live on main."*
Gary's ruling removes the "fund main" clause: cash can come from **any** managed ledger (incl. main).
...
### 8.3 Candidate design (PROPOSED — needs sign-off)
...
### 8.4 Open questions for the governor (BLOCKING the design)
