...
> **RESUME HERE → RUN (ledger-money gate) — **PR7 complete 2026-09-20** (`dao_protocol` #175, `94d0b142`; the `dao_client` package lives in `dao_protocol` — the `dao_client` repo redirects, same as PR5's #174). New CLI `truesight-dao-batch-link-sunmint` (`truesight_dao_client/modules/batch_link_sunmint.py`): `compute_allocations()` pairs the SOLD‑unlinked QR backlog to eligible units per **Decision 0.5** — a **distinct, not-yet-used owner email → a photographed tree (1:1)**; a **repeat email or a blank email → a plot association**, where a plot is **reusable / round-robin** (a plot spans many trees' worth of supply, so it is *not* consumed like a tree). Drives the existing `[TREE PLANTING LINK EVENT]` once per pair via `EdgarClient.from_env().submit()` — **dry-run is the default** and `--execute` **refuses without `--yes`** (rc=2, fires nothing), because the link event books ledger rows (the §2 ledger-money gate). Catalog: `TREE PLANTING LINK EVENT` gains the **`Plot ID`** label (PR6 had shipped it in GAS/dapp only) with `required_fields` relaxed to `[QR Code]` (either/or target). Verified: `ruff check`/`format --check` clean, new pure-core tests **12/12**, full `dao_protocol` pytest **200 passed** (main baseline 188; the 5 failures + 2 collection errors pre-existing). **No ledger event, no `clasp` deploy.** Next: **RUN** — Paulo's 10-tree backfill (§1.1) + first real batch link — which stays under the **ledger-money gate (§2): explicit governor go only.**
>
> **PR3 shipped 2026-09-20 — `tokenomics` #531 (`4a6b5c0`), GAS source-only (no deploy):** `reconcileTreePlanting_()` added to `process_tree_planting_telegram_logs.js`; both `appendRow` paths (file-attached and no-attachment) now call it when `rowStatus === 'NEW'`. Match key = `Contributor Name` col J (§1.7); balance = signed sum of that farmer's rows on the `Cacao Tree Purchased - Not Planted` literal, FIFO cursor = count of prior `-1` rows. **Path A** (open balance) → `-1 Purchased-Not-Planted` / `+1 Planted-Unassigned` on the main `offchain transactions` tab + stamps the SunMint row's `Payment Event Ref` (col U, header created once if absent) with the consumed purchase's `[ASSET RECEIPT EVENT]` id (joined via the `Asset Receipts` audit tab's `Offchain Row` column). **Path B** (no balance) → `+1 Cacao Tree - To Be Paid For` on main always (§0.3). Mirrors the proven `appendTreePlantingLedgerFulfillment_` writer; literal line-items, no `Currencies` row; never throws; skips `Unknown`; no backfill sweep. Verified: `node --check`, no dup consts, 2 call sites (L328/L399), `pytest scripts/` **79 passed**, focused `node` FIFO/Path-A/B sim **7/7**, diff = 1 file **+142**. **No `clasp push`/deploy** and no ledger-writing event executed (both always-stop gates).
>
> **PR4 shipped 2026-09-20 — `tokenomics` #533 (`376dc3a`) / #534 (`2dacfaf`) / #535 (`96207b0`) + `dao_protocol` #174 (`cd97a65`); GAS source-only (no deploy):** three steps. (1) **Pure legs** (#533): `fpeBuildLegs_` computes the ledger legs per §0.11/§1.2. (2) **Wiring + I/O** (#534): `fpeResolveQrLedgerUrl_` / `fpeResolveLedgerSpreadsheetUrl_` / `fpeFindSunMintRow_` (join key = the SunMint row’s col D `Telegram Message ID`, matching the link handler’s own `TPL_SUNMINT_MESSAGE_ID_COL`); `fpeBookLedger_` runs **before** `appendPayoutEventRow_(…,false)` so the Tier-1 row carries the true outcome — `BOOKED`, or `LEDGER_NOT_BOOKED` + reason; wrapped so a ledger hiccup never aborts the payout scan. **Fails closed** on no-tree-id / no-SunMint-row / unresolvable-QR-ledger / bad-amount. (3) **`dao_protocol`** (#174): verified `[PAYOUT EVENT]` was already routed (#170) **and already catalogued with `Tree Planting IDs`**, so **no `[FARMER PAYMENT EVENT]` tag and no routing entry were needed** — the plan’s anticipated “confirmation”; PR4 extended the catalog semantics + added a regression pin. **Partial-write coverage** (#535, Envoy review): the deliberate no-rollback-on-partial-failure behaviour is now an executable harness case (`PARTIAL_WRITE_N_OF_M` ⇒ flagged, landed legs asserted not rolled back) rather than implicit (§5.9c TC11). Verified: `node --check`, harness **37/37**, `tokenomics` pytest **81→86**, `dao_protocol` targeted **17/17** (pre-existing unrelated failures identical to main). **No `clasp push`/deploy** and no ledger event executed (both always-stop gates).
