...
**RESUME HERE → PR5** (PR0 + PR0b + PR1 + PR2 + PR3 + PR4 complete 2026-09-17 — §7 pre-flight resolved; architecture = **Option B**). PR1 = `sunmint_beta` vendor-readiness (`sunmint_beta` #84, sha 3732e2a4). PR2 = the vendor PR into `cfr-anapu` `gh-pages` (`cfr-anapu` #5, sha f0ffdbda; **live on `cfr.truesight.me`**, smoke-check green). PR3 = the `lineage-engine` sync job (`lineage-engine` #23, sha 81e61956; dry-run default — writes `programs/<slug>/pk-<hash>/sunmint/*.json` into `lineage-credentials`). PR4 = the `build_cv_cache.py` `sunmint/` activity kind + `program_modes` (`lineage-engine` #24, sha 07486ae0; 6 new tests, suite 25 passed). **PR5 = `program-shell.js` SunMint badges + itemized click-through in `truesight_me_beta` (+ `cfr-anapu` mirror), beta-only** (§2.2 pts 5–6; prod gated on UAT). **PR6 remains the only `gate: human`.**
