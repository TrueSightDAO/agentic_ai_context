...
| **PR4** | ✅ **DONE 2026-09-17** (`lineage-engine` #24, sha `07486ae0`) — extended `build_cv_cache.py`: `collect_practitioners()` folds the new `programs/<slug>/pk-<hash>/sunmint/*.json` kind into per-program `trees_planted_count` / `monitoring_events_count` / `plots_registered_count` / `sunmint_event_count` / `last_sunmint_activity_at`; `build_unified_cv()` surfaces those + the itemized `sunmint_events[]` (cap 20) and `has_elective_records` is now true off SunMint alone; new `_program_modes(manifest)` tolerates **both** the `program_modes` array and the legacy `program_mode` string (§6 #4); new tested `program_activity_score()` counts SunMint so a tree-only contributor still resolves a `primary_program` (never dropped from `_cache/index.json`); `render_markdown()` guards the practice lines behind `practice_count` (kills the "Total practice time: 0 minutes" hazard, §1.4) and adds a SunMint section. Practice semantics untouched. `tests/test_build_cv_cache_sunmint.py` (6 tests); full suite **25 passed**. | auto |
| **PR5** | ✅ **DONE 2026-09-17** (`truesight_me_beta` #381 + `lineage-engine` #25, sha `df5b0ba`; backend `member_sunmint_aggregate()` + uniform-shape `setdefault`, end-to-end `build()` guard, suite **29 passed**) — `program-shell.js` renders SunMint activity badges on `members.html` cards (§2.2 point 5, aggregate-only per §4) **and** extends the `credentials/index.html` CV renderer to list itemized `sunmint_events[]` rows on click-through, each linking to the tree's public Impact Map/QR entry rather than duplicating GPS/photo (§2.2 point 6, §4). Update `programs/crf-anapu/manifest.json` to the new `program_modes` shape. | auto (beta); prod gated on UAT |
| **PR6** | ⏳ **BLOCKED (verified 2026-09-17)** — precondition not met: `sync_sunmint_program_activity.py --dry-run` reads 168 `tree_planting` / 2 `tree_growth_monitoring` / 19 `farm_boundary_evidence_event` source events in `verify_public_signatures`, but **0 attributable** to `crf-anapu` (no real `cfr.truesight.me` submission has landed yet — the vendored app went live in PR2). Needs at least one real submission first (may require a CEPOTX/Jedielcio coordination step outside any PR). Then: dry-run → `--push` (live) → verify `members.html` populates. | **`gate: human`** — first live write into `lineage-credentials` from a new activity kind |
| **PR7** | ✅ **DONE 2026-09-17** (`agentic_ai_context` PR — docs) — `CREDENTIALING_PROGRAM_PAGES.md` §6 documents the non-exclusive `program_modes` array (`cohort_credentialing` / `sunmint_cohort`) + the `_program_modes()` legacy-`program_mode` backward-compat + the badge/CV fields it drives (first instance: `programs/crf-anapu/manifest.json`); `CRF_ANAPU_MEDIA_TASK_PLAN.md` gains a cross-reference progress entry. UAT itself is §9 (governor-driven, not a PR). | auto |

PR7 = docs (`CREDENTIALING_PROGRAM_PAGES.md` §6 `program_modes`/`sunmint_cohort` + `CRF_ANAPU_MEDIA_TASK_PLAN.md` cross-ref) shipped 2026-09-17. **RESUME HERE → PR6** (PR0 + PR0b + PR1 + PR2 + PR3 + PR4 + PR5 + PR7 complete 2026-09-17 — §7 pre-flight resolved; architecture = **Option B**). PR1 = `sunmint_beta` vendor-readiness (`sunmint_beta` #84, sha 3732e2a4). PR2 = the vendor PR into `cfr-anapu` `gh-pages` (`cfr-anapu` #5, sha f0ffdbda; **live on `cfr.truesight.me`**, smoke-check green). PR3 = the `lineage-engine` sync job (`lineage-engine` #23, sha 81e61956; dry-run default — writes `programs/<slug>/pk-<hash>/sunmint/*.json` into `lineage-credentials`). PR4 = the `build_cv_cache.py` `sunmint/` activity kind + `program_modes` (`lineage-engine` #24, sha 07486ae0; 6 new tests, suite 25 passed). PR5 = `program-shell.js` SunMint badges + itemized click-through in `truesight_me_beta` #381 (+ `crf-anapu` mirror, byte-identical) **plus** the `lineage-engine` #25 flat `sunmint_*` aggregate emission (sha `df5b0ba`) that feeds the renderer, beta-only (§2.2 pts 5–6; prod gated on UAT). **PR6 remains the only `gate: human`.**
...
**Approved; PR0 + PR0b + PR1 + PR2 + PR3 + PR4 + PR5 + PR7 complete; PR6 is the only unit left and it is `gate: human` — blocked on the first real submission.** Reviewed in the Telegram topic **"CFR partnership - figure
