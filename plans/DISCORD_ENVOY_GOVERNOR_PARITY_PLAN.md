...
**RESUME HERE = COMPLETE (2026-09-17).** PR0 ✅ (bot id + sheet col-G/col-W binding confirmed) → PR1 ✅ (trusted-bot carve-out, `truesight_autopilot` #491, live) → PR2 ✅ shipped via the env mechanism **path (i)** — `DISCORD_SENTINEL_USER_IDS` + `DISCORD_TRUSTED_BOT_IDS` both live; `author_role("1548902290344255600")` → `sentinel` verified on-box — plus the durable deploy-env staleness fix `truesight_autopilot` **#494** (so env-only changes now actually deploy) → PR3 ✅ UAT (Gary confirmed a real Discord reply from Envoy's bot; the security/regression U-checks are covered by PR1's unit tests). The underlying cache bug (Finding A, email-keyed `sentinel_emails()`) is left as the **non-blocking residual** in `OPEN_FOLLOWUPS.md`.
