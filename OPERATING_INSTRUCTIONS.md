...
| — | **PLACARD_GENERATION_SOP.md** | **Event placard generation.** When the governor asks for a branded table placard with QR code for an event (e.g. SF Tech Fest, farmers market), read this SOP and use `/opt/truesight_autopilot/scripts/placard_generator.py`. Covers QR registration, PNG generation, upload to lineage-assets, and troubleshooting. |
