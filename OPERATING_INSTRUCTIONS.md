...
| — | **LOCAL_EMULATOR_SETUP.md** | **How to spin up the local Android emulator + iOS Simulator on this Mac for mobile-app testing** (Xcode/SDK locations, one-time install, env vars, create/boot, build + install + launch, gotchas). Read when asked to run a mobile app locally. |
...
| — | **DAO_CLIENT_AI_AGENT_CONTRIBUTIONS.md** | **AI → Edgar `[CONTRIBUTION EVENT]`** convention (PR URLs — or a commit URL when work was merged directly with no PR — explicit body, `dao_client/.env`). **Also covers the `[CONTRIBUTION EVENT]` vs `[CAPITAL INJECTION EVENT]` distinction** — use CONTRIBUTION for time/out-of-pocket expenses; use CAPITAL INJECTION only for external investors wiring into AGL contracts. |
